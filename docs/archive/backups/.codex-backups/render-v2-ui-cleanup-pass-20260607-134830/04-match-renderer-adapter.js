(function(){
  ...
  function drawPile(ctx, pile){
    ...
    if(!isDeck) {
      ctx.save();
      roundedPath(ctx, cardR.x + 4, cardR.y + 4, Math.max(1, cardR.w - 8), Math.max(1, cardR.h - 8), Math.max(4, cardR.w * .05));
      ctx.strokeStyle = 'rgba(255,226,105,.66)';
      ctx.lineWidth = 1.2;
      ctx.stroke();
      ctx.restore();
    }
  ...
  function drawCommandDock(ctx, hitMap, snapshot, cssW, cssH){
    ...
    sealGrad.addColorStop(0, canEndTurn ? 'rgba(255,245,181,.86)' : 'rgba(160,166,172,.38)');
    sealGrad.addColorStop(.34, canEndTurn ? 'rgba(204,148,48,.96)' : 'rgba(62,66,72,.80)');
    sealGrad.addColorStop(.66, canEndTurn ? 'rgba(66,42,16,.98)' : 'rgba(31,33,38,.92)');
    sealGrad.addColorStop(1, canEndTurn ? 'rgba(9,12,18,.98)' : 'rgba(9,11,16,.96)');
    ctx.beginPath();
    for(let i = 0; i < 8; i++){
    ...
    ctx.shadowColor = canEndTurn ? 'rgba(255,210,88,.28)' : 'transparent';
    ctx.shadowBlur = canEndTurn ? 16 : 0;
    ctx.fill();
    ctx.shadowBlur = 0;
    ctx.lineWidth = 1.4;
    ...
    ctx.strokeStyle = canEndTurn ? 'rgba(255,246,188,.34)' : 'rgba(210,210,204,.16)';
    ctx.lineWidth = 1.1;
    ctx.stroke();
    drawCommandGlyph(ctx, 'end-turn', {x:sealCx - 14, y:sealCy - 19, w:32, h:32}, canEndTurn ? 'rgba(255,244,180,.96)' : 'rgba(220,220,212,.38)');
    ctx.fillStyle = canEndTurn ? 'rgba(255,238,156,.86)' : 'rgba(210,210,204,.36)';
    ctx.font = '900 7px Cinzel, serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('END', sealCx, sealCy + 15);
    ctx.fillText('TURN', sealCx, sealCy + 24);
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText('COMMANDS', x + 92, y + 29);
    ctx.fillStyle = 'rgba(255,232,132,.42)';
    ctx.font = '700 8px Cinzel, serif';
    ctx.fillText(interaction.consolidating ? 'TRIBUTE SELECTION ACTIVE' : (canEndTurn ? 'END TURN ON THE SEAL' : 'OPPONENT IN MOTION'), x + 92, y + 45);
    ...
    const smallH = 36;
    const row1Y = y + 88;
    const row2Y = row1Y + smallH + 9;
    const row3Y = row2Y + smallH + 9;
    const colW = (innerW - gap) / 2;
    const buttons = [];
    if(interaction.consolidating) buttons.push({command:'consolidate', label:'STOP', active:true});
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText(interaction.consolidating ? 'Select reinforcement cards' : 'Chat, audio, and match controls', innerX + 2, y + h - 13);
    ctx.restore();
  }
