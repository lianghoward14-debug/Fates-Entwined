(function(){
  ...
  function drawCommandDock(ctx, hitMap, snapshot, cssW, cssH){
    ...
    ctx.lineWidth = 1.1;
    ctx.stroke();
    drawCommandGlyph(ctx, 'end-turn', {x:sealCx - 13, y:sealCy - 18, w:30, h:30}, canEndTurn ? 'rgba(255,244,180,.96)' : 'rgba(220,220,212,.38)');
    ctx.fillStyle = canEndTurn ? 'rgba(255,238,156,.86)' : 'rgba(210,210,204,.36)';
    ctx.font = '900 7px Cinzel, serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('END', sealCx, sealCy + 13);
    ctx.fillText('TURN', sealCx, sealCy + 22);
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText('END TURN', x + 88, y + 30);
    ctx.fillStyle = 'rgba(255,232,132,.42)';
    ctx.font = '700 8px Cinzel, serif';
    ctx.fillText(interaction.consolidating ? 'TRIBUTE SELECTION' : (canEndTurn ? 'SEAL BUTTON READY' : 'WAITING'), x + 88, y + 46);
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText(interaction.consolidating ? 'Select reinforcement cards' : 'Audio, chat, and match controls', innerX + 2, y + h - 11);
    ctx.restore();
  }
