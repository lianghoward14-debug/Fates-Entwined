(function(){
  ...
  function drawHandPanel(ctx, handRect, snapshot, count){
    ...
    ctx.fill();
    ctx.lineWidth = 1.35;
    ctx.strokeStyle = 'rgba(226,196,96,.62)';
    ctx.stroke();
    roundedPath(ctx, r.x + 10, r.y + 10, Math.max(1, r.w - 20), Math.max(1, r.h - 20), 5);
    ctx.lineWidth = 1.35;
    ctx.strokeStyle = 'rgba(255,226,105,.38)';
    ctx.stroke();
    drawCornerRails(ctx, {x:r.x + 17, y:r.y + 17, w:Math.max(1, r.w - 34), h:Math.max(1, r.h - 34)}, 'rgba(255,226,105,.46)', .72);
  ...
  function drawCommandDock(ctx, hitMap, snapshot, cssW, cssH){
    ...
    bg.addColorStop(0, 'rgba(45,34,14,.96)');
    bg.addColorStop(.18, 'rgba(8,9,12,.98)');
    bg.addColorStop(.72, 'rgba(5,6,10,.98)');
    bg.addColorStop(1, 'rgba(30,20,10,.94)');
    ctx.fillStyle = bg;
    ctx.fill();
    ctx.strokeStyle = 'rgba(255,222,92,.88)';
    ctx.lineWidth = 1.55;
    ctx.stroke();
    ctx.strokeStyle = 'rgba(255,246,188,.18)';
    ctx.lineWidth = 1;
    roundedPath(ctx, x + 8, y + 8, w - 16, h - 16, 5);
    ctx.stroke();

    const sealCx = x + 60;
    const sealCy = y + h / 2;
    const sealR = 36;
    const sealGrad = ctx.createRadialGradient(sealCx - 8, sealCy - 9, 4, sealCx, sealCy, sealR);
    sealGrad.addColorStop(0, 'rgba(130,197,230,.72)');
    sealGrad.addColorStop(.36, 'rgba(24,57,72,.86)');
    sealGrad.addColorStop(.62, 'rgba(55,37,19,.98)');
    sealGrad.addColorStop(1, 'rgba(176,130,45,.94)');
    ctx.beginPath();
    for(let i = 0; i < 8; i++){
      const a = -Math.PI / 2 + i * Math.PI / 4;
      const rr = i % 2 ? sealR * .86 : sealR;
      const px = sealCx + Math.cos(a) * rr;
      const py = sealCy + Math.sin(a) * rr;
    ...
    ctx.fill();
    ctx.lineWidth = 2;
    ctx.strokeStyle = 'rgba(255,226,128,.78)';
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(sealCx, sealCy, sealR - 11, 0, Math.PI * 2);
    ctx.strokeStyle = 'rgba(255,246,188,.25)';
    ctx.lineWidth = 1.1;
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(sealCx - 7, sealCy - 14);
    ctx.lineTo(sealCx + 15, sealCy);
    ctx.lineTo(sealCx - 7, sealCy + 14);
    ctx.closePath();
    ctx.fillStyle = interaction.consolidating ? 'rgba(139,244,196,.94)' : 'rgba(255,244,180,.92)';
    ctx.fill();

    ctx.strokeStyle = 'rgba(255,226,105,.34)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(x + 108, y + 13);
    ctx.lineTo(x + 97, y + h - 13);
    ctx.stroke();
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('COMMAND', sealCx, y + h - 21);
    ctx.fillStyle = 'rgba(255,232,132,.42)';
    ctx.font = '700 7px Cinzel, serif';
    ctx.fillText(interaction.consolidating ? 'ACTIVE' : 'MATCH', sealCx, y + h - 10);
    ...
    drawCommandButton(ctx, hitMap, 'end-turn', 'END TURN', {x:innerX, y:row1Y, w:innerW, h:primaryH}, {
      primary:true,
      disabled:interaction.currentPlayer !== snapshot.viewer
    });
    ctx.save();
    ctx.globalAlpha = .36;
    ctx.strokeStyle = 'rgba(255,226,105,.42)';
    ctx.beginPath();
    ctx.moveTo(innerX + 12, y + 53);
    ctx.lineTo(innerX + innerW - 12, y + 53);
    ctx.stroke();
    ctx.restore();
    const buttons = [];
    if(canConsolidate){
      buttons.push({command:'consolidate', label:interaction.consolidating ? 'STOP' : 'CONSOLIDATE', active:interaction.consolidating});
    }
    buttons.push({command:'audio', label:'AUDIO'});
    buttons.push({command:'world-chat', label:'WORLD'});
    buttons.push({command:'end-game', label:'END GAME', danger:true});
    const colW = (innerW - smallGap * Math.max(0, buttons.length - 1)) / Math.max(1, buttons.length);
    buttons.forEach(function(button, index){
      drawCommandButton(ctx, hitMap, button.command, button.label, {x:innerX + index * (colW + smallGap), y:row2Y, w:colW, h:smallH}, {
    ...
    if(canConsolidate){
      ctx.fillStyle = interaction.consolidating ? 'rgba(143,244,196,.76)' : 'rgba(255,232,132,.54)';
      ctx.font = '700 7px Cinzel, serif';
      ctx.textAlign = 'left';
      ctx.textBaseline = 'middle';
      ctx.fillText(interaction.consolidating ? 'Choose tribute cards' : 'Ready for orders', innerX + 2, y + h - 12);
    } else {
      ctx.fillStyle = 'rgba(255,232,132,.40)';
      ctx.font = '700 7px Cinzel, serif';
      ctx.textAlign = 'left';
      ctx.textBaseline = 'middle';
      ctx.fillText('Waiting for your main phase', innerX + 2, y + h - 12);
    }
    ctx.restore();
  }
