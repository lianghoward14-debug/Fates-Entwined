(function(){
  ...
  function drawCommandButton(ctx, hitMap, command, label, rect, opts){
    ...
    ctx.font = '800 ' + Math.max(9, Math.round(rect.h * (compact ? .28 : .25))) + 'px Cinzel, serif';
  ...
  function drawCommandDock(ctx, hitMap, snapshot, cssW, cssH){
    ...
    const w = clamp(cssW * .235, 286, 334);
    const h = 214;
    ...
    ctx.lineWidth = 1.45;
    ctx.stroke();
    roundedPath(ctx, x + 9, y + 9, w - 18, h - 18, 8);
    ctx.strokeStyle = 'rgba(255,246,188,.16)';
    ctx.lineWidth = 1;
    ctx.stroke();
    ...
    sealGrad.addColorStop(0, 'rgba(150,214,242,.76)');
    sealGrad.addColorStop(.40, 'rgba(25,66,82,.88)');
    sealGrad.addColorStop(.72, 'rgba(58,40,20,.97)');
    sealGrad.addColorStop(1, 'rgba(195,142,48,.95)');
    ctx.beginPath();
    for(let i = 0; i < 8; i++){
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(sealCx - 5, sealCy - 9);
    ctx.lineTo(sealCx + 10, sealCy);
    ctx.lineTo(sealCx - 5, sealCy + 9);
    ctx.closePath();
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText('COMMANDS', x + 76, y + 29);
    ctx.fillStyle = 'rgba(255,232,132,.42)';
    ctx.font = '700 8px Cinzel, serif';
    ctx.fillText(interaction.consolidating ? 'TRIBUTE SELECTION ACTIVE' : (interaction.currentPlayer === snapshot.viewer ? 'AWAITING YOUR ORDER' : 'OPPONENT IN MOTION'), x + 76, y + 45);
    ...
    ctx.stroke();
    drawCornerRails(ctx, {x:x + 18, y:y + 18, w:w - 36, h:h - 36}, 'rgba(255,226,105,.42)', .72);
    ...
    drawCommandButton(ctx, hitMap, 'end-turn', 'END TURN', {x:innerX, y:row1Y, w:innerW, h:primaryH}, {
      ...
      subLabel:interaction.currentPlayer === snapshot.viewer ? 'pass priority' : 'locked'
    });
    const colW = (innerW - gap) / 2;
    const buttons = [
      {command:'consolidate', label:interaction.consolidating ? 'STOP' : 'CONSOLIDATE', active:interaction.consolidating, disabled:!canConsolidate},
      {command:'audio', label:'AUDIO'},
      {command:'world-chat', label:'CHAT'},
      {command:'end-game', label:'END GAME', danger:true}
    ];
    buttons.forEach(function(button, index){
      const col = index % 2;
      const row = Math.floor(index / 2);
      drawCommandButton(ctx, hitMap, button.command, button.label, {x:innerX + col * (colW + gap), y:(row === 0 ? row2Y : row3Y), w:colW, h:smallH}, {
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText(interaction.consolidating ? 'Choose tribute cards' : (canConsolidate ? 'Main phase options ready' : 'Consolidate unlocks during main phase'), innerX + 2, y + h - 13);
    ctx.restore();
  }
