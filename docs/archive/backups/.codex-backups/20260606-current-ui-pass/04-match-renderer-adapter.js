(function(){
  ...
  function drawTributeCue(ctx, r, state){
    ...
    const color = ready ? 'rgba(126,218,255,1)' : selected ? 'rgba(255,239,110,1)' : placement ? 'rgba(255,228,95,1)' : 'rgba(255,221,68,.98)';
    const fill = ready ? 'rgba(82,182,255,.24)' : selected ? 'rgba(255,218,62,.22)' : placement ? 'rgba(255,215,0,.18)' : 'rgba(255,215,0,.13)';
    const radius = Math.max(5, Math.min(10, r.w * .06));
    const outerPad = (selected || ready || placement) ? 5.2 : 4.2;
    ctx.save();
    ctx.shadowColor = ready ? 'rgba(104,199,255,.70)' : 'rgba(255,220,70,.68)';
    ctx.shadowBlur = ready ? 24 : 20;
    roundedPath(ctx, r.x - outerPad, r.y - outerPad, r.w + outerPad * 2, r.h + outerPad * 2, radius + 2);
    ctx.fillStyle = fill;
    ctx.fill();
    ctx.lineWidth = ready ? 3.6 : (selected ? 3.4 : (placement ? 3 : 2.65));
  ...
  function handHoverRect(r){
    const scale = 1.075;
    const w = r.w * scale;
    const h = r.h * scale;
    return {
      x:r.x + r.w / 2 - w / 2,
      y:r.y - Math.max(8, r.h * .06) - (h - r.h) * .8,
      w,
      h
    };
  }

  function drawHandHoverFrame(ctx, r){
    ctx.save();
    ctx.shadowColor = 'rgba(255,217,92,.48)';
    ctx.shadowBlur = 15;
    ctx.lineWidth = 1.4;
    ctx.strokeStyle = 'rgba(255,227,132,.86)';
    roundedPath(ctx, r.x - 3, r.y - 3, r.w + 6, r.h + 6, Math.max(6, r.w * .055));
    ctx.stroke();
    ctx.shadowBlur = 0;
    ctx.globalAlpha = .28;
    ctx.strokeStyle = 'rgba(95,181,255,.65)';
    ctx.lineWidth = 1;
    roundedPath(ctx, r.x - 6, r.y - 6, r.w + 12, r.h + 12, Math.max(7, r.w * .06));
    ctx.stroke();
    ctx.restore();
  }
  ...
  function drawCommandDock(ctx, hitMap, snapshot, cssW, cssH){
    const w = Math.min(184, Math.max(162, cssW * .095));
    const h = 238;
    const x = Math.max(12, cssW - w - 18);
    const y = Math.max(92, cssH - h - 118);
    const r = {x, y, w, h};
    const interaction = snapshot && snapshot.interaction ? snapshot.interaction : {};
    ctx.save();
    roundedPath(ctx, x, y, w, h, 10);
    const bg = ctx.createLinearGradient(x, y, x, y + h);
    bg.addColorStop(0, 'rgba(25,21,14,.90)');
    bg.addColorStop(.42, 'rgba(8,10,16,.94)');
    bg.addColorStop(1, 'rgba(4,5,9,.97)');
    ctx.fillStyle = bg;
    ctx.fill();
    ctx.strokeStyle = 'rgba(236,203,101,.78)';
    ctx.lineWidth = 1.6;
    ctx.stroke();
    ctx.strokeStyle = 'rgba(255,247,194,.20)';
    ctx.lineWidth = 1;
    roundedPath(ctx, x + 6, y + 6, w - 12, h - 12, 8);
    ctx.stroke();
    drawCornerRails(ctx, r, 'rgba(255,223,91,.88)', 1);
    ctx.globalAlpha = .56;
    ctx.strokeStyle = 'rgba(236,203,101,.36)';
    ctx.beginPath();
    ctx.moveTo(x + 18, y + 44);
    ctx.lineTo(x + w - 18, y + 44);
    ctx.moveTo(x + 18, y + h - 42);
    ctx.lineTo(x + w - 18, y + h - 42);
    ctx.stroke();
    ctx.globalAlpha = 1;
    ctx.fillStyle = '#f2d778';
    ctx.font = '900 10px Cinzel, serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('COMMAND', x + w / 2, y + 24);
    ...
    drawCommandButton(ctx, hitMap, 'end-turn', 'END TURN', {x:btnX, y:y + 58, w:btnW, h:38}, {primary:true, disabled:interaction.currentPlayer !== snapshot.viewer});
    drawCommandButton(ctx, hitMap, 'consolidate', interaction.consolidating ? 'CANCEL' : 'CONSOLIDATE', {x:btnX, y:y + 106, w:btnW, h:34}, {disabled:consolidateDisabled, active:interaction.consolidating});
    drawCommandButton(ctx, hitMap, 'end-game', 'END GAME', {x:btnX, y:y + 150, w:btnW, h:34}, {danger:true, disabled:false});
    drawCommandButton(ctx, hitMap, 'audio', 'AUDIO', {x:btnX, y:y + 196, w:btnW, h:24}, {disabled:false});
    ctx.restore();
  }
  ...
  function drawSceneUi(ctx, layout, snapshot, cssW, cssH){
    ...
    function drawHandItem(item){
      ...
      const drawRect = isHovered ? handHoverRect(item.rect) : item.rect;
      if(isHovered) drawHandHoverFrame(ctx, drawRect);
      drawCardVisual(ctx, {card:item.card, c:item.index, r:0, z:0}, visual, drawRect, function(){ scheduleTextureRender('hand-texture-ready'); }, {pulse:false, tilt:0, lift:isHovered ? .95 : 0, hideFateBadge:true});
      if(disabled) drawDisabledCardOverlay(ctx, drawRect, 'SET LIMIT');
