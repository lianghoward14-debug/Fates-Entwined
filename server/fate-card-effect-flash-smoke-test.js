...
assert.match(rooms, /transactionOwnsPicker = opts\?\.onlineParentAction === true[\s\S]*effectTransactions\.isActive\(\)[\s\S]*_onlineClientOwnedBoardActionPickerDepth > 0 \|\| transactionOwnsPicker[\s\S]*originals\.pickCardsVisual/, 'only explicitly awaited or transaction-owned pickers may bypass the nested multiplayer picker action');
assert.match(core, /case '42'[\s\S]*await drawCard[\s\S]*await waitForEffectPresentationBeforeChoice\(\)[\s\S]*pickCardsVisual/, 'West German Soldier must finish the draw presentation before opening its forced discard picker');
...
assert.match(core, /const currentPlayer = G\.currentPlayer[\s\S]*await drawCard\(currentPlayer, 1,[\s\S]*cardActsAsPassive\(card, '46'\) && card\.owner===currentPlayer[\s\S]*'phil_crown'/, 'Phil and valid passive copies must grow and flash only during the captured owner Draw Phase');
...
assert.match(core, /function coordinatorAuraAffectsTarget[\s\S]*cardActsAsPassive\(source, '10'\)[\s\S]*target\.owner !== source\.owner[\s\S]*cardActsAsPassive\(source, '01'\)[\s\S]*Math\.abs[\s\S]*cardActsAsPassive\(source, '11'\)[\s\S]*isCardSupporterForRules[\s\S]*cardActsAsPassive\(source, '19'\)[\s\S]*target\.type === 'Coordinator'[\s\S]*cardActsAsPassive\(source, '23'\)[\s\S]*isCardCharacterForRules/, 'each continuous coordinator placement sigil, including valid passive copies, must target the cards that actually receive that zone aura');
...
assert.match(core, /function getCoordinatorPlacementFlashTargets[\s\S]*cardActsAsPassive\(source, '57'\)[\s\S]*coordinatorAuraAffectsTarget\(aura[\s\S]*receivesBoostedAura[\s\S]*window\.getCoordinatorPlacementFlashTargets/, 'Jeremiah and valid passive copies must flash the cards receiving boosted coordinator auras, not merely the coordinator sources');
...
assert.match(adapter, /presentFateDelta\(\{[\s\S]*flashIncomingCoordinatorEffects\(record\.iid[\s\S]*incomingCoordinatorFeedback[\s\S]*kvetkaOnlyGain[\s\S]*playSfx\(delta > 0 \? 'fateGain' : 'fateLose'\)/, 'incoming Coordinator overlays and their combined Fate gain or decrease must begin in one reveal callback');
...
const coordinatorRuntime = {
  ...
  cardActsAsPassive:(card, sourceId)=>String(card?.id || '') === String(sourceId),
...
assert.match(core, /triggerRozsiPassive[\s\S]*const amount = 3 \+ boost[\s\S]*modifyFate\(card, amount, 'permanent'\)[\s\S]*rozsi_dance/, 'Rozsi and boosted copied-passive overlay gains must use the ordinary Fate-gain sound');
...
assert.match(core, /cardActsAsPassive\(card, '46'\)[\s\S]*const before[\s\S]*playFateChangeSound\(card, before, card\.currentFate, currentPlayer\)[\s\S]*phil_crown/, 'Phil and valid passive-copy direct Fate gains must play the ordinary Fate-gain sound');
...
assert.match(rooms, /function maybeFlashOnlineTargetEffectDeltas\(action[\s\S]*effectTransactionVersion\) === 1\) return false[\s\S]*british_union_jack[\s\S]*isaac_beaker[\s\S]*oathbound_crescent[\s\S]*scheduleOnlineClientCardFlash\(\s*after\.card,\s*kind/, 'legacy multiplayer actions must reconstruct 17th British, Isaac, and Oathbound overlays, while transaction-owned actions use their attached presentation event');
...
assert.match(rooms, /function maybePlayOnlineRemoteBoardChangeAudio[\s\S]*kvetkaCoordinatorGain[\s\S]*String\(entry\.card\.id \|\| ''\) === '19'[\s\S]*rendererOwnsFateFeedback[\s\S]*fateUp && !kvetkaCoordinatorGain[\s\S]*'FATE_GAIN'/, 'multiplayer must retain generic Fate-gain audio for non-Kvetka overlays while suppressing duplicates owned by Kvetka or the renderer');
