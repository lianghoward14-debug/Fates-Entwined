...
assert.match(core, /function coordinatorAuraAffectsTarget[\s\S]*source\.id === '10'[\s\S]*target\.owner !== source\.owner[\s\S]*source\.id === '01'[\s\S]*Math\.abs[\s\S]*source\.id === '11'[\s\S]*isCardSupporterForRules[\s\S]*source\.id === '15'[\s\S]*return true[\s\S]*source\.id === '19'[\s\S]*target\.type === 'Coordinator'[\s\S]*source\.id === '23'[\s\S]*isCardCharacterForRules/, 'each coordinator placement sigil must target the cards that actually receive that zone aura');
