...
assert.match(renderer, /A Noble Effort at a Ballad is active: your consolidations gain 3 Fate permanently until you set a Supporter[\s\S]*printedEffect: card \? card\.effect/, 'Kvetka Ballad must retain its complete internal active-effect description before dropdown presentation cleanup');
...
assert.match(css, /#tp-status-right \.effect-pill:not\(\.effect-pill-overflow\) \.effect-pill-icon,[\s\S]*\.effect-pill\.effect-pill-opp:not\(\.effect-pill-overflow\) \.effect-pill-icon[\s\S]*border:1px solid rgba\(255,122,132,\.66\)[\s\S]*rgba\(225,70,82,\.18\)/, 'opponent status-banner icon frames must use the opponent red outline instead of the local blue');
