...
assert.match(indexText, /05-gameplay-core\.js\?v=1782744100/, 'index must cache-bust gameplay consolidation, Oathbound, end-turn warning, timer sync, and Rivera buff fixes');
...
assert.match(indexText, /18-online-rooms\.js\?v=1782744100/, 'index must cache-bust the online rooms script for server card picker identity, Oathbound targeting, and authority timer sync fixes');
assert.match(indexText, /09-challenger-mode\.js\?v=1782663700/, 'index must cache-bust the challenger leaderboard, simulation, social filtering, profile crop, and browser deck art script');
...
assert.match(indexText, /render-v2\/04-match-renderer-adapter\.js\?v=1782620100/, 'index must cache-bust render-v2 pending effect visibility fixes');
assert.match(indexText, /19-online-elo\.js\?v=1782623300/, 'index must cache-bust the online ELO script for full-pair Fly Challenger AI fixes');
assert.match(indexText, /21-smoothness-core\.js\?v=1782616800/, 'index must cache-bust the smoothness script for warmup and startup-finished event changes');
...
assert.match(authorityText, /function runFlyAISimulationBatch\(monthKey[\s\S]*cadenceMs = 10 \* 60 \* 1000[\s\S]*listFlyLiveMatches\(1\)[\s\S]*pairs = flyAIPairs[\s\S]*Math\.min\(pairs\.length/, 'Fly Challenger AI simulation must be globally cadenced, full-pair capped, and skipped during live matches');
...
assert.match(setupText, /function runAISimulation\(options=\{\}\)[\s\S]*fate_ai_sim_cadence[\s\S]*DOMContentLoaded[\s\S]*setTimeout\(\(\) =>[\s\S]*runAISimulation\(\)[\s\S]*setInterval\(\(\) =>[\s\S]*runAISimulation\(\)/, 'local AI simulations must run on load');
...
assert.match(read('src/scripts/19-online-elo.js'), /\/api\/challenger-ai\/seed[\s\S]*pairCount[\s\S]*\/api\/challenger-ai\/simulate[\s\S]*count:pairCount[\s\S]*startSharedAISimulationLoop\(\)/, 'online ELO bridge must use full-pair Fly Challenger AI endpoints');
