...
assert.match(moraleUi, /const busserEvent=String\(event&&event\.semanticSourceCardId\|\|'\'\)==='69'[\s\S]{0,220}if\(!busserEvent\|\|String\(event&&event\.type\|\|'\'\)\.toUpperCase\(\)!=='MORALE_HEALED'\)return null/, 'ordinary Morale source overlays must be limited to the explicitly designed Busser effect');
