...
const jaime = catalog.byId.get('bh22');
assert(jaime, 'expected Jaime in the shared single-player and multiplayer catalog');
assert.strictEqual(jaime.cost, 1, 'Jaime must cost 1 Reinforcement in single-player and multiplayer');
assert.strictEqual(jaime.effect, 'Select a square in your safe row. At every Morale calculation, recover Morale equal to the Fate of the card on that square.', 'Jaime card text must remain synchronized across single-player and multiplayer');
