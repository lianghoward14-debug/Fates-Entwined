...
const WEBSITE_PUBLIC_PREFIX = '/website';
...
const GAME_STATIC_PREFIXES = [
  '/src/',
  '/optimized/',
  '/titlscreenbackgrounds/',
  '/ingamebackgrouds/',
  '/setvoicelines/',
  '/new voices/',
  '/soundeffects/',
...
function safeNullableNumber(value, min, max){
  if(value === null || value === undefined || value === '') return null;
  const n = Number(value);
...
function noStoreStaticPath(filePath){
  const ext = path.extname(filePath).toLowerCase();
  return ext === '.html' || ext === '.js' || ext === '.css' || ext === '.json' || ext === '.webmanifest' || path.basename(filePath).toLowerCase() === 'sw.js';
}
...
function serveFile(res, filePath, downloadName){
  fs.stat(filePath, (err, stat)=>{
    ...
    const headers = {
      ...
      'cache-control':downloadName || noStoreStaticPath(filePath) ? 'no-store, no-cache, must-revalidate, max-age=0' : 'public, max-age=300'
    };
    if(downloadName || noStoreStaticPath(filePath)){
...
function normalizePublicStaticPath(publicPath){
  let pathname = String(publicPath || '').trim();
  if(!pathname) return '';
  try{
...
function isAllowedStaticOverridePath(publicPath){
  const pathname = normalizePublicStaticPath(publicPath);
  if(!pathname) return false;
  if(pathname === INSTALLER_PUBLIC_PATH) return false;
  if(pathname === WEBSITE_PUBLIC_PREFIX || pathname.startsWith(WEBSITE_PUBLIC_PREFIX + '/')) return true;
  return isAllowedGameStaticPath(pathname);
}
...
function serveStaticFileWithOverride(res, baseDir, pathname, publicPath){
  const overridePath = staticOverridePath(publicPath || pathname);
  if(overridePath && fs.existsSync(overridePath) && fs.statSync(overridePath).isFile()){
    serveFile(res, overridePath);
    return true;
  }
  return serveStaticFile(res, baseDir, pathname);
}

function writeStaticOverride(publicPath, body){
  if(!STATIC_HOTFIX_TOKEN) throw new Error('static hotfix uploads are disabled');
  if(String(body?.token || '') !== STATIC_HOTFIX_TOKEN) throw new Error('invalid static hotfix token');
  const normalizedPath = normalizePublicStaticPath(publicPath || body?.path);
  if(!isAllowedStaticOverridePath(normalizedPath)) throw new Error('static hotfix path is not allowed');
  const target = staticOverridePath(normalizedPath);
  if(!target) throw new Error('static hotfix storage is unavailable');
  const encoding = String(body?.encoding || 'utf8').toLowerCase();
  const raw = body?.content;
  if(typeof raw !== 'string') throw new Error('static hotfix content must be a string');
  const bytes = encoding === 'base64' ? Buffer.from(raw, 'base64') : Buffer.from(raw, 'utf8');
  if(bytes.length > MAX_HTTP_BODY_BYTES) throw new Error('static hotfix file is too large');
  fs.mkdirSync(path.dirname(target), {recursive:true});
  const tmp = target + '.tmp-' + process.pid + '-' + Date.now();
  fs.writeFileSync(tmp, bytes);
  fs.renameSync(tmp, target);
  return {path:normalizedPath, bytes:bytes.length};
}
...
function serveStaticFile(res, baseDir, pathname){
  const target = safeStaticPath(baseDir, pathname);
  if(!target){
...
function isAllowedGameStaticPath(pathname){
  if(pathname === '/') return true;
  if(GAME_ROOT_FILES.has(pathname)) return true;
  if(GAME_STATIC_PREFIXES.some(prefix=>pathname.startsWith(prefix))) return true;
  return /^\/[^/]+\.(?:png|jpg|jpeg|webp|svg|json|txt|ico|webmanifest)$/i.test(pathname);
}
...
function serveWebsiteRequest(req, res){
  ...
  if(pathname === WEBSITE_PUBLIC_PREFIX || pathname === WEBSITE_PUBLIC_PREFIX + '/'){
    return serveStaticFileWithOverride(res, WEBSITE_DIR, '/index.html', WEBSITE_PUBLIC_PREFIX + '/index.html');
  }
  if(pathname.startsWith(WEBSITE_PUBLIC_PREFIX + '/')){
    const websitePath = pathname.slice(WEBSITE_PUBLIC_PREFIX.length) || '/index.html';
    return serveStaticFileWithOverride(res, WEBSITE_DIR, websitePath, pathname);
  }
  if(pathname === '/') pathname = '/index.html';
  if(!isAllowedGameStaticPath(pathname)) return false;
  return serveStaticFileWithOverride(res, GAME_DIR, pathname, pathname);
}
...
function sanitizeProfile(value){
  ...
  return {
    ...
    profileCropFocusX:safeNullableNumber(profile.profileCropFocusX, 0, 1),
    profileCropFocusY:safeNullableNumber(profile.profileCropFocusY, 0, 1),
    profileCropZoom:safeNullableNumber(profile.profileCropZoom, 1, 4),
    profileCropY:safeNullableNumber(profile.profileCropY, -100, 100),
...
function publicFlyProfile(value){
  ...
  return {
    ...
    profileCropFocusX:safeNullableNumber(raw.profileCropFocusX, 0, 1),
    profileCropFocusY:safeNullableNumber(raw.profileCropFocusY, 0, 1),
    profileCropZoom:safeNullableNumber(raw.profileCropZoom, 1, 4),
    profileCropY:safeNullableNumber(raw.profileCropY, -100, 100),
...
function listFlyAIRecords(monthKey = ''){
  const key = String(monthKey || '').slice(0, 24);
  return [...flyAIRecords.values()]
    .map(publicFlyAIRecord)
    .filter(Boolean)
    .filter(record=>!key || record.monthKey === key)
    .sort((a,b)=>{
      const eloDelta = (Number(b.elo || 0) || 0) - (Number(a.elo || 0) || 0);
      if(eloDelta) return eloDelta;
      return String(a.name || '').localeCompare(String(b.name || ''));
    });
}

function seedFlyAIRecords(roster, monthKey = ''){
  const key = String(monthKey || '').slice(0, 24);
  const incoming = (Array.isArray(roster) ? roster : []).slice(0, 40)
    .map(item=>publicFlyAIRecord(Object.assign({}, item, {monthKey:key || item?.monthKey || ''})))
    .filter(Boolean);
  if(!incoming.length) return listFlyAIRecords(key);
  const incomingIds = new Set(incoming.map(record=>record.aiId));
  for(const [id, existing] of [...flyAIRecords.entries()]){
    if(existing?.isMonthly && key && existing.monthKey !== key) flyAIRecords.delete(id);
  }
  incoming.forEach(record=>{
    ...
    flyAIRecords.set(record.aiId, publicFlyAIRecord(Object.assign({}, existing, {
  ...
  for(const [id, existing] of [...flyAIRecords.entries()]){
    if(existing?.isMonthly && key && existing.monthKey === key && !incomingIds.has(id)) flyAIRecords.delete(id);
  }
  persistFlyRoomMutation();
  return listFlyAIRecords(key);
}

function flyAIWinChance(a, b, seed){
  const trueA = Number(a.trueElo || a.elo || 600) || 600;
  const trueB = Number(b.trueElo || b.elo || 600) || 600;
  const expected = 1 / (1 + Math.pow(10, (trueB - trueA) / 400));
  const wobbleRng = makeSeededRng(`${seed}:${a.aiId}:${b.aiId}`);
  const wobble = (wobbleRng() - 0.5) * 0.06;
  return Math.max(0.03, Math.min(0.97, expected + wobble));
}

function flyAIPairs(records, rng){
  const ids = records.map(record=>record.aiId);
...
function runFlyAISimulationBatch(monthKey = '', count = 4){
  const key = String(monthKey || '').slice(0, 24);
  ...
  const maxCount = Math.min(4, Math.max(1, Math.round(Number(count || 4) || 4)));
  ...
  for(let i=0; i<Math.min(maxCount, pairs.length); i++){
    ...
    if(!a || !b) continue;
    const chanceA = flyAIWinChance(a, b, `${key}:${i}`);
    const aWins = rng() < chanceA;
    const expectedA = 1 / (1 + Math.pow(10, (Number(b.elo || 600) - Number(a.elo || 600)) / 400));
    const k = 24;
    let changeA = Math.round(k * ((aWins ? 1 : 0) - expectedA));
    if(aWins && changeA < 1) changeA = 1;
    if(!aWins && changeA > -1) changeA = -1;
    const changeB = -changeA;
    a.elo = a.challengerElo = Math.max(100, Math.round(Number(a.elo || 600) + changeA));
    b.elo = b.challengerElo = Math.max(100, Math.round(Number(b.elo || 600) + changeB));
    a.wins = a.challengerWins = safeNonNegativeInteger(a.wins, 0) + (aWins ? 1 : 0);
    a.losses = a.challengerLosses = safeNonNegativeInteger(a.losses, 0) + (aWins ? 0 : 1);
    b.wins = b.challengerWins = safeNonNegativeInteger(b.wins, 0) + (aWins ? 0 : 1);
    b.losses = b.challengerLosses = safeNonNegativeInteger(b.losses, 0) + (aWins ? 1 : 0);
    a.matchesPlayed = safeNonNegativeInteger(a.matchesPlayed, 0) + 1;
    b.matchesPlayed = safeNonNegativeInteger(b.matchesPlayed, 0) + 1;
    a.updatedAt = b.updatedAt = now();
    flyAIRecords.set(a.aiId, publicFlyAIRecord(a));
    flyAIRecords.set(b.aiId, publicFlyAIRecord(b));
    ran++;
  }
  flyAISimSchedule = {monthKey:key, lastRunAt:now(), lastRan:ran, updatedAt:now()};
  if(ran) persistFlyRoomMutation();
  return {ran, skipped:'', roster:listFlyAIRecords(key)};
}

function flyLeaderboard(limit = 100){
  const humanEntries = [...flyPlayerStats.values()]
    .map(publicFlyProfile)
    .filter(Boolean)
  ...
  const aiEntries = [...flyAIRecords.values()]
    .map(publicFlyAIRecord)
    .filter(Boolean);
  return [...humanEntries, ...aiEntries]
    .sort((a,b)=>{
...
function deckChoiceValidationError(deckChoice, label){
  return validateDeckIds(deckChoice?.deckIds || [], authorityCardCatalog(), label || 'deck');
}
...
function findFlyMatchmakingOpponent(uid, mode, partyTargetUid){
  ...
  for(const entry of candidates){
    ...
    const hostDeckErr = deckChoiceValidationError(host.deckChoice, 'queued host deck');
    if(hostDeckErr){
      console.warn(`Removing invalid matchmaking host ${entry.uid}: ${hostDeckErr}`);
      removeMatchmakingEntry(entry.uid);
      continue;
    }
    return {entry, room};
  }
...
async function enterFlyMatchmaking(uid, body){
  ...
  const deckErr = deckChoiceValidationError(deckChoice, 'queue deck');
  if(deckErr) throw new Error(deckErr);
  ...
  if(match){
    ...
    let accepted = null;
    try{
      accepted = await startRoomOnFly(room, room.hostUid, {
        seed:`${room.code}_queue_${now()}_${crypto.randomBytes(4).toString('hex')}`,
        mode
      });
    }catch(e){
...
function leaveFlyRoom(room, uid){
  ...
  if(player){
    ...
    player.lastSeen = now();
    if(shouldFinalizeDisconnectImmediately(room)){
      clearDisconnectTimer(room, uid);
      finalizeDisconnectTimeout(room.code, uid).catch(err=>{
        console.error('Immediate leave finalization failed:', err.message || err);
      });
    }else{
...
function validateAction(room, ws, msg){
  ...
  if(type !== 'FORFEIT' && type !== 'CHOOSE_TURN' && type !== 'STATE_SYNC' && type !== 'REACTION_CHOICE' && type !== 'PICK_LANDSCAPE_ZONE' && type !== 'RESOLVE_ZONE_PICK'){
...
async function handleApiRequest(req, res, url){
  ...
  try{
    ...
    if(parts[1] === 'challenger-ai'){
      if(req.method === 'GET'){
        const monthKey = String(url.searchParams.get('monthKey') || '').slice(0, 24);
        writeJson(res, 200, {ok:true, roster:listFlyAIRecords(monthKey), schedule:Object.assign({}, flyAISimSchedule || {})});
        return true;
      }
      if(req.method === 'POST' && parts[2] === 'seed'){
      ...
      if(req.method === 'POST' && parts[2] === 'simulate'){
        const body = await readJsonBody(req);
        await verifyRequestUser(req, body);
    ...
    if(parts[1] === 'matchmaking'){
      if(req.method === 'POST' && parts[2] === 'enter'){
        const body = await readJsonBody(req);
        const uid = await verifyRequestUser(req, body);
        const result = await enterFlyMatchmaking(uid, body);
        writeJson(res, 200, {
          ok:true,
