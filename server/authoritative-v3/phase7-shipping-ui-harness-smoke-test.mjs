...
assert.match(onlineRooms, /BOARD_DESTINATION[\s\S]{0,4200}phase7GuardOptionPrompt\(promptKey, prompt\)/, 'board destination prompts must remount their shipping picker after a late modal close');
assert.match(onlineRooms, /BOARD_TARGET[\s\S]{0,2200}phase7GuardOptionPrompt\(promptKey, prompt\)/, 'board target prompts must remount their shipping picker after a late modal close');
...
assert.match(onlineRooms, /if\(!fateMotionShown && window\.FateMatchRendererAdapter[\s\S]{0,900}presentFateDelta[\s\S]{0,900}fromValue:fateBefore[\s\S]{0,900}toValue:(?:baseFateAfter|fateAfter)/, 'a busy VFX recipe must fall back to the production Fate-number adapter instead of dropping visible Fate feedback');
...
assert.match(onlineRooms, /fateChangeAtLocation\(target, pos, fateBefore, (?:baseFateAfter|fateAfter), fateOptions\)/, 'Phase 7 must route non-board Fate changes through the off-board production motion path');
assert.match(onlineRooms, /const resultFeedbackFrameAt = Date\.now\(\)[\s\S]{0,8000}phase7ShowExactEffectOverlay\(view, event, target, eventIndex, resultFeedbackFrameAt\)[\s\S]{0,12000}fate-motion:start[\s\S]{0,500}resultFeedbackFrameAt/, 'the exact overlay and Fate number motion must share a paint-frame timestamp, with the overlay installed first');
...
assert.match(onlineRooms, /pickerNode = document\.querySelector\('#modal \.board-target-picker, #modal \.zone-picker-wrap'\)[\s\S]{0,700}getClientRects\(\)\.length > 0[\s\S]{0,1200}pickerKey = ''/, 'a hidden or replaced production board picker must reopen for the unchanged authoritative prompt');
...
assert.match(onlineRooms, /Card inspection or another incidental modal can replace a live target[\s\S]{0,500}pickerKey = ''/, 'a submitted or replaced board picker must clear its guard so an unchanged prompt can recover after a no-op');
