...
assert.strictEqual(catalog.byId.get('82').cost, 3, 'Felicyta Youth must match the new 3-Reinforcement art');
assert.strictEqual(catalog.byId.get('85').cost, 4, 'Felicyta Specters must match the new 4-Reinforcement art');
...
for(const cardId of ['38','54','71','82','85','96','97']) {
  const full = path.join(root, cardId + '.png');
  const thumb = path.join(root, 'optimized/card-thumbs', cardId + '.jpg');
  const pfp = path.join(root, 'pfp', 'pfp' + cardId + '.png');
  assert(fs.existsSync(thumb), 'card ' + cardId + ' optimized thumbnail must exist');
  assert(fs.statSync(thumb).mtimeMs >= fs.statSync(full).mtimeMs, 'card ' + cardId + ' optimized thumbnail must be regenerated from its current art');
  assert(fs.existsSync(pfp), 'card ' + cardId + ' profile crop must exist');
  assert(fs.statSync(pfp).mtimeMs >= fs.statSync(full).mtimeMs, 'card ' + cardId + ' profile crop must be regenerated from its current art');
}
...
assert.match(gameSetup, /watcher\.lastRevealedName = card\.name/, 'Fort Calvin Watcher must remember the revealed card name');
assert.match(gameSetup, /watcher\.sentCharacterName = card\.name/, 'Fort Calvin Watcher must remember the bottom-decked Character name');
assert.match(gameSetup, /source\._fortCalvinSentCharacterName = watcher\.sentCharacterName/, 'Fort Calvin Watcher must copy the bottom-decked card name back to its card information tracker');
assert.match(rendering, /card\.id \|\| ''\) === '71'[\s\S]*Card Revealed[\s\S]*Sent to Bottom:[\s\S]*Last Revealed:/, 'Fort Calvin Watcher card information must use card-wide reveal wording and name the bottom-decked Character');
assert.match(rendering, /Waiting for Opponent\\'s Draw Phase/, 'Fort Calvin Watcher Match Tracker must name the opponent draw phase while waiting');
assert.match(rendering, /card\.id \|\| ''\) === '71'[\s\S]*value = sentName \|\| lastName \|\| 'None'/, 'Fort Calvin Watcher Match Tracker must show None until a Character is revealed');
assert.match(rendering, /card\.id \|\| ''\) === '71'[\s\S]*cd-fort-calvin-tracker/, 'Fort Calvin Watcher Match Tracker must expose a specific class for subline positioning');
assert.match(lastCss, /cd-fort-calvin-tracker \.cd-live-tracker-sub[\s\S]{0,120}transform:translateY\(-2px\)/, 'Fort Calvin Watcher sent-to-bottom subline must sit 2px higher');
assert.doesNotMatch(rendering, /Character reveal limit reached:[\s\S]*was sent to the bottom of the deck/, 'Fort Calvin Watcher top status banner must stay compact and not carry the named-card tracker text');
assert.match(rendering, /showZoneTitles[\s\S]*pickerClass[\s\S]*board-target-zone-title/, 'the shared field picker must support explicit polished zone grouping for Jake');
...
assert.match(rooms, /maybePlayOnlineNewCharacterCinematic[\s\S]*onlineCardIsCharacter\(entry\.card\)[\s\S]*requestCharacterSetCinematic/, 'multiplayer Character sets must use the same shared cinematic');
