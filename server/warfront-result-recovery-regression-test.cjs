...
let writes=0;
const lifecycle=await import('./authoritative-v3/warfront-lifecycle.mjs');
const c={...lifecycle,clone:structuredClone,warfrontBindings:new Map([['match',{mapCode:'map',zoneId:'front',uids:['bravo','alpha']}]]),warfrontEvent:{mapCode:'map',zones:[zone]},persist:()=>writes++,applyChallengerResult:()=>{}};
...
zone.a={uid:'alpha'};zone.b={uid:'bravo'};c.warfrontBindings.get('match').settled=false;zone.matches=[];zone.activeMatch={matchId:'match'};match.outcome={winner:null,type:'DRAW'};
