(function(){
  ...
  function drawCommandDock(ctx, hitMap, snapshot, cssW, cssH){
    ...
    const sealCy = y + 40;
    const sealR = 28;
    const sealGrad = ctx.createRadialGradient(sealCx - 9, sealCy - 10, 3, sealCx, sealCy, sealR);
    sealGrad.addColorStop(0, canEndTurn ? 'rgba(255,249,198,.95)' : 'rgba(160,166,172,.38)');
    sealGrad.addColorStop(.27, canEndTurn ? 'rgba(205,154,50,.96)' : 'rgba(62,66,72,.80)');
    sealGrad.addColorStop(.55, canEndTurn ? 'rgba(41,49,41,.98)' : 'rgba(31,33,38,.92)');
    sealGrad.addColorStop(.78, canEndTurn ? 'rgba(15,22,23,.98)' : 'rgba(9,11,16,.96)');
    ...
    ctx.strokeStyle = canEndTurn ? 'rgba(255,239,147,.92)' : 'rgba(220,220,212,.38)';
    ctx.lineWidth = 2.4;
    ctx.lineCap = 'round';
    ctx.lineJoin = 'round';
    ctx.beginPath();
    ctx.arc(0, 0, sealR - 15, -Math.PI * .78, Math.PI * .78);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(5, -9);
    ctx.lineTo(17, 0);
    ...
    ctx.font = '700 8px Cinzel, serif';
    ctx.fillText(interaction.consolidating ? 'TRIBUTE SELECTION' : (canEndTurn ? 'READY' : 'WAITING'), x + 88, y + 46);
