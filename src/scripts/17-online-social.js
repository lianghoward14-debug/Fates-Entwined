...
(function(){
  ...
  function photoStyle(p, fallback='center 22%'){
      ...
      <div class="profile-wrap title-profile-modal-v2 inspected-profile-stable">
        <div class="profile-left-col">
          <div class="profile-img-wrap" style="${rankFrame}">
            <img src="${esc(photoOf(p))}" style="${esc(photoStyle(p))}" alt="" onerror="this.onerror=null;this.src='blank.png';">
          </div>
        </div>
  ...
  function friendRow(uid){
    ...
    return `<div class="social-friend-row" onclick="window.inspectOnlineProfile('${esc(uid)}')">
      <div class="social-friend-pic" style="${frameStyle}"><img src="${esc(photoOf(p))}" style="${esc(photoStyle(p))}" onerror="this.onerror=null;this.src='blank.png';"></div>
  ...
  function requestRow(uid){
    const p = profileOf(uid);
    return `<div class="social-online-row online-request-row"><div class="social-online-pic"><img src="${esc(photoOf(p))}" style="${esc(photoStyle(p))}" onerror="this.onerror=null;this.src='blank.png';"></div><div class="social-online-info"><div>${esc(nameOf(p))}</div><div>${esc(p.baseCode||'')}</div></div><button class="btn sm pri" onclick="event.stopPropagation();this.closest('.online-request-row')?.classList.add('is-resolving');window.acceptOnlineFriend('${esc(uid)}')">Accept</button><button class="btn sm" onclick="event.stopPropagation();this.closest('.online-request-row')?.classList.add('is-resolving');window.declineOnlineFriend('${esc(uid)}')">Decline</button></div>`;
  }
  function onlineCard(uid){
    ...
    return `<div class="social-online-card" onclick="window.inspectOnlineProfile('${esc(uid)}')">
      ...
      <div class="social-online-card-pic" style="${frameStyle}"><img src="${esc(photoOf(p))}" style="${esc(photoStyle(p))}" onerror="this.onerror=null;this.src='blank.png';"><span class="social-status-dot"></span></div>
  ...
  function partyMemberRow(uid, status){
    ...
    return `<div class="party-member-clean">
      <div class="party-member-avatar"><img src="${esc(photoOf(p))}" style="${esc(photoStyle(p))}" onerror="this.onerror=null;this.src='blank.png';"></div>
  ...
  function partyInviteRow(uid){
    ...
    return `<div class="social-online-row online-party-request">
      <div class="social-online-pic"><img src="${esc(photoOf(p))}" style="${esc(photoStyle(p))}" onerror="this.onerror=null;this.src='blank.png';"></div>
  ...
  function dmMessageHtml(m, peer){
    ...
    const senderStyle = isMe ? photoStyle(currentProfile()) : photoStyle(m.photoURL ? {profileImg:m.photoURL} : peer);
    return `<div class="social-dm ${isMe ? 'social-dm-me' : 'social-dm-them'}">
      <div class="social-dm-avatar"><img src="${esc(senderPic || 'blank.png')}" style="${esc(senderStyle)}" onerror="this.onerror=null;this.src='blank.png';"></div>
  ...
  function renderDmMessagesOnly(){
    ...
    if(headerPic) {
      ...
      headerPic.style.cssText = photoStyle(peer);
    }
  }
    ...
    if(!dmShellOpen || forceShell || !document.getElementById('dm-input')){
      bodyEl.innerHTML = `
        <div class="social-dm-header">
          <div class="social-dm-header-pic" style="${typeof window.getRankFrameStyle==='function'?window.getRankFrameStyle(peerElo,'icon'):''}"><img src="${esc(peerPic)}" style="${esc(photoStyle(peer))}" onerror="this.onerror=null;this.src='blank.png';"></div>
