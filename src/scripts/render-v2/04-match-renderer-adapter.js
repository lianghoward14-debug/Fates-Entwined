(function(){
  ...
  function drawDisabledCardOverlay(ctx, r, label){
    const pad = Math.max(3, r.w * .035);
    const rr = {x:r.x - pad, y:r.y - pad, w:r.w + pad * 2, h:r.h + pad * 2};
    ctx.save();
    roundedPath(ctx, rr.x, rr.y, rr.w, rr.h, Math.max(5, r.w * .06));
    ctx.clip();
    ctx.fillStyle = 'rgba(18,20,24,.76)';
    ctx.fillRect(rr.x, rr.y, rr.w, rr.h);
    ctx.globalCompositeOperation = 'saturation';
    ctx.fillStyle = 'rgba(120,120,120,1)';
    ctx.fillRect(rr.x, rr.y, rr.w, rr.h);
    ctx.globalCompositeOperation = 'source-over';
    ctx.fillStyle = 'rgba(8,9,12,.34)';
    ctx.fillRect(rr.x, rr.y, rr.w, rr.h);
  ...
  function getInGameChatUnreadCount(){
    try{
      if(typeof window.fateDebugOnlineUi === 'function'){
        const state = window.fateDebugOnlineUi() || {};
        return Math.max(0, (Number(state.chatUnread || 0) || 0) + (Number(state.worldUnread || 0) || 0));
      }
    }catch(e){}
    try{
      const widget = document.getElementById('ingame-chat-widget');
      if(widget && widget.classList.contains('has-chat-unread')) return 1;
      const badge = document.getElementById('ingame-chat-badge');
      if(badge && badge.style.display !== 'none') return 1;
    }catch(e){}
    return 0;
  }

  function drawCommandNotifyDot(ctx, rect){
    ctx.save();
    const radius = Math.max(4, Math.min(6, rect.h * .16));
    const cx = rect.x + rect.w - radius - 12;
    const cy = rect.y + radius + 6;
    ctx.shadowColor = 'rgba(255,48,56,.72)';
    ctx.shadowBlur = 11;
    ...
    ctx.shadowBlur = 0;
    ctx.lineWidth = 1.2;
    ctx.strokeStyle = 'rgba(255,238,226,.94)';
    ctx.stroke();
    ctx.restore();
  }
  ...
  function drawCommandButton(ctx, hitMap, command, label, rect, opts){
    ...
    if(options.notify && !disabled) drawCommandNotifyDot(ctx, rect);
    ctx.restore();
  ...
  function drawCommandDock(ctx, hitMap, snapshot, cssW, cssH){
    ...
    buttons.push({command:'world-chat', label:'CHAT', notify:getInGameChatUnreadCount() > 0});
  ...
  function drawSceneUi(ctx, layout, snapshot, cssW, cssH){
    ...
    function drawHandItem(item){
      ...
      const iconRect = drawHandEffectIcon(ctx, item.card, item.rect);
      if(iconRect) hitMap.handEffectIcons.push({kind:'hand-effect-icon', index:item.index, iid:item.iid, rect:iconRect, iconRect, cardRect:item.rect, card:item.card, effects:handEffectRows(item.card)});
  ...
  function enqueueRender(source, dirtyMask){
    ...
    if(srcLower.indexOf('post-action-deferred') < 0 && !isActionAnimationActive() && wasActionAnimatingRecently(180) && heavyActionDirtyMask(nextMask)) {
