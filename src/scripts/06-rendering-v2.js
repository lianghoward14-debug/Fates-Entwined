...
function getHandRenderSignature() {
  ...
  const viewingOwn = (Number(cp) === Number(G.currentPlayer));
  ...
  if(G.erbsActive) {
    for(let p = 0; p < 2; p++) {
      if(G.erbsActive[p]) {
        ...
        allEffects.push({
          icon: getStatusEffectIcon('ready'), label: 'Erbs Ready',
  ...
  if((fromHand||G.selectedHandCard!==null) && isLocalHandCard){
    if(card.id==='70'){
      if(!card.guerilla_transferred){
        ...
        guer.dataset.unavailable = canUseHandCard ? '' : '1';
        guer.title = canUseHandCard ? 'Activate Effect' : explainUnavailableHandAction();
        guer.onclick=()=>{
