...
function resolveLocalPhotoValue(value){
  if(!value) return '';
  const appendCrop = (src) => {
    if(!src || !value || typeof value !== 'object') return src;
    const hasCrop = value.cropFocusX !== undefined || value.cropFocusY !== undefined || value.cropZoom !== undefined || value.cropY !== undefined || value.cropTx !== undefined || value.cropTy !== undefined;
    if(!hasCrop) return src;
    const fx = Math.max(0, Math.min(1000, Math.round(Number(value.cropFocusX ?? 0.5) * 1000)));
    const fy = Math.max(0, Math.min(1000, Math.round(Number(value.cropFocusY ?? 0.5) * 1000)));
    const z = Math.max(100, Math.min(400, Math.round(Number(value.cropZoom || 1) * 100)));
    const sep = String(src).includes('?') ? '&' : '?';
    return `${src}${sep}fc=${fx},${fy},${z}`;
  };
  if(value && typeof value === 'object'){
    if(value.pfpId) return appendCrop('pfp/pfp' + (Math.max(1, parseInt(value.pfpId, 10) || 1)) + '.png');
    if(value.cardImg) return appendCrop(value.cardImg);
    if(value.src && !String(value.src).startsWith('data:')) return appendCrop(value.src);
...
function getLocalPhoto(user){
  const p = getLocalProfile();
  return resolveLocalPhotoValue(p.profileImg)
    || resolveLocalPhotoValue(p.photoURL)
    || resolveLocalPhotoValue(p.pfp)
    || resolveLocalPhotoValue(user?.photoURL)
    || 'blank.png';
}
...
async function syncPublicProfile(opts={}){
  ...
  const payload = {
    ...
    photoURL,
    profileImg: photoURL,
    profileCropFocusX: localProfile?.profileImg && typeof localProfile.profileImg === 'object' ? localProfile.profileImg.cropFocusX : null,
    profileCropFocusY: localProfile?.profileImg && typeof localProfile.profileImg === 'object' ? localProfile.profileImg.cropFocusY : null,
    profileCropZoom: localProfile?.profileImg && typeof localProfile.profileImg === 'object' ? localProfile.profileImg.cropZoom : null,
    profileCropY: localProfile?.profileImg && typeof localProfile.profileImg === 'object' ? localProfile.profileImg.cropY : null,
    level: getLocalLevel(),
