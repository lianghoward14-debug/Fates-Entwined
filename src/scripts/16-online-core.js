...
(function(){
  ...
  window.FateOnline = Object.assign(window.FateOnline || {}, {
    ...
    renderTinyProfile(p){
      return `<div class="fo-profile-tiny"><img src="${esc(profilePhoto(p))}" style="${esc(profilePhotoCropStyle(p))}" onerror="this.onerror=null;this.src='blank.png';"><span>${esc(profileName(p))}</span></div>`;
    }
  });
})();
