...
function renderSocialPage() {
  ...
  const partyHtml = party ? party.members.map(m => {
    ...
    return `<div class="social-party-member">
      <div class="social-party-pic">
        ${memberPic ? `<img src="${memberPic}" loading="eager" decoding="async" draggable="false" style="${memberCrop}">` : `<span style="font-size:1rem;color:var(--dim);">${m.username.charAt(0).toUpperCase()}</span>`}
      </div>
      <div style="flex:1;min-width:0;">
          ...
          : `<div class="social-friend-list" style="overflow:hidden;">${pageFriends.map(f => {
            ...
            <div class="social-friend-row" onclick='inspectProfile(${jsString(f.username)})'>
              <div class="social-friend-pic">
                ${friendPic ? `<img src="${friendPic}" loading="eager" decoding="async" draggable="false" style="${friendCrop}">` : `<span style="font-size:1rem;color:var(--dim);">${f.username.charAt(0).toUpperCase()}</span>`}
...
function updateInGameChatBadge() {
  ...
  if(widget) widget.classList.toggle('has-chat-unread', totalUnread > 0);
  if(toggle) toggle.classList.toggle('has-chat-unread', totalUnread > 0);
  if(toggle){
    ...
    dot.classList.toggle('is-on', totalUnread > 0);
  ...
  if(totalUnread > 0) {
    badge.style.display = 'inline-flex';
    badge.textContent = '';
    badge.setAttribute('aria-label', totalUnread > 1 ? `${totalUnread} unread messages` : 'Unread message');
...
window.fateDebugOnlineUi = function fateDebugOnlineUi(){
  ...
  return {
    ...
    chatTab:_inGameChatTab,
    chatUnread:_inGameChatUnread,
    worldUnread:_worldChatUnread,
