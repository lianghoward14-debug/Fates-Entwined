...
function isTurnEndDeferrableModalOpen() {
  ...
  const onlinePlayerIndex = Number(G._onlinePlayerIndex);
  const hasOnlinePlayerIndex = G._onlinePlayerIndex !== undefined && G._onlinePlayerIndex !== null && G._onlinePlayerIndex !== '';
  if(G._onlineRoomCode && hasOnlinePlayerIndex && Number.isInteger(onlinePlayerIndex) && Number(G.currentPlayer) !== onlinePlayerIndex) return false;
  if(!G._onlineRoomCode) {
    ...
    if(Number.isInteger(Number(viewer)) && Number(G.currentPlayer) !== Number(viewer)) return false;
  }
  return true;
}
...
function getCurrentActionPlayerForEndTurnWarning() {
  ...
  if(G._onlineRoomCode) {
    const localIdx = Number(G.localPlayerIndex);
    const onlineIdx = Number(G._onlinePlayerIndex);
    const idx = Number.isInteger(localIdx) ? localIdx : onlineIdx;
    return Number.isInteger(idx) && idx === Number(G.currentPlayer) ? idx : null;
  }
  if(G.aiEnabled && (G.currentPlayer === G.aiPlayer || G._aiRunning)) return null;
  const viewer = typeof getPerspectivePlayerIndex === 'function' ? getPerspectivePlayerIndex() : G.currentPlayer;
  return Number.isInteger(Number(viewer)) && Number(viewer) === Number(G.currentPlayer) ? Number(viewer) : G.currentPlayer;
}
...
function handleConsolidateClick(z,r,c) {
  ...
  if(con.phase==='select_placement'){
    ...
    const selectedReinforcement = con.chosenIdxs.reduce((s,i)=>s+(con.allPossible[i].reinforcement||1),0);
...
async function runWhenSetEffect(inst, z, r, c) {
  ...
  // Mark supporter as having used its when-set effect after suppression/landscape limits pass.
