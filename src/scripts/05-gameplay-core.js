...
function finalizeConsolidate(card, tributes, targetIdx, conContext) {
  ...
  function finishConsolidate(useFaceDown) {
    ...
    let bonusFate = 0;
    tributes.forEach(t=>{
      if(t.card.id==='47' && !(typeof isSupporterEffectSuppressed === 'function' && isSupporterEffectSuppressed(t.card))) bonusFate += 3;
...
function getEffectiveFate(card, z) {
  ...
  // Bobby Jones (55): +7 Fate if all own cards in zone share same affiliation
  if(card.id==='55'){
    let allSameAff = true;
    let ownAff = null;
    let ownCount = 0;
    ...
    if(allSameAff && ownAff && ownCount >= 3) bonus += 7;
  }
