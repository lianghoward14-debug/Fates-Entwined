...
function recordFreePlayResult(didWin, opponentElo=1000) {
  ...
  const drops = awardVictoryDrops(didWin);
  saveProfile();
...
function recordChallengerResult(didWin, opponentElo=1000, isAI=false) {
...
function showStarterPick() {
  ...
  STARTER_DECKS.forEach(deck=>{
    ...
    el.innerHTML = `
      <div class="starter-deck-art">${img?`<img src="${img}" alt="" onerror="this.style.display='none'">`:''}</div>
...
function renderChallengerDeckPickModal(page=0) {
  ...
  pageKeys.forEach(pid=>{
    ...
    tile.innerHTML = `
      <div class="preset-tile-art">
        ${hero?.img ? `<img src="${hero.img}" alt="${escapeHtml(hero.name)}" onerror="this.style.display='none'">` : ''}
  ...
  showModal(modalTitle, '', modalActions);
  const modalBox = document.querySelector('#modal .modal');
  if(modalBox) modalBox.classList.add('choose-deck-canonical-modal');
...
function renderFreePlayTitlePresetDeckPickModal(page=0) {
  ...
  pageKeys.forEach((pid,i)=>{
    ...
    const heroArt = hero?.img ? `<img src="${hero.img}" alt="${escapeHtml(hero.name)}" onerror="this.parentElement.style.display='none'">` : '';
    el.innerHTML = `
  ...
  body.appendChild(footer);
  showModal('Choose Your Deck', '', CURRENT_MODE === 'free' ? [] : [{label:'Cancel', action:closeModal}]);
  const modalBox = document.querySelector('#modal .modal');
  if(modalBox) modalBox.classList.add('freeplay-title-preset-modal');
  document.getElementById('modal-body').innerHTML = '';
  document.getElementById('modal-body').appendChild(body);
}
...
function showProfilePackOpening(pfpIds) {
  ...
  for(let i=0;i<20;i++){
    ...
    s.className = 'pack-sparkle';
    s.style.left = Math.random()*100+'%';
    s.style.top = Math.random()*100+'%';
    s.style.animationDelay = (Math.random()*2.6)+'s';
    s.style.background = '#7fb6ff';
    s.style.boxShadow = '0 0 8px #7fb6ff, 0 0 18px rgba(127,182,255,.75)';
...
function showPackOpening(results, packType) {
  ...
  // Spawn sparkles
  sparkleLayer.innerHTML = '';
  for(let i=0;i<(isFavored?36:24);i++){
    const s = document.createElement('div');
    s.className='pack-sparkle' + (isFavored?' pack-sparkle-gold':'');
    s.style.left = Math.random()*100+'%';
    s.style.top = Math.random()*100+'%';
...
function renderProfilePackReveal(pfpIds) {
  ...
  pfpIds.forEach((pfpId, idx)=>{
    ...
    wrap.innerHTML = `
      <div class="pack-card-face pack-card-front rarity-circle" style="border-color:#7fb6ff;box-shadow:0 0 28px rgba(127,182,255,.55),0 0 60px rgba(127,182,255,.2);">
  ...
  content.innerHTML = `
    <div class="cdb-command-panel">
      ...
      <div class="cdb-command-actions">
        ...
        ${_cdbCurrentDeckId ? `<button class="btn sm" style="border-color:var(--gold);color:var(--gold);" onclick="cdbOverwriteDeck()">Overwrite Current</button>` : ''}
...
function browseChallengerDecks(selectedPid=null, page=_challengerDeckBrowsePage) {
  ...
  document.getElementById('modal-acts').appendChild(close);
  const modalBox = document.querySelector('#modal .modal');
  if(modalBox) modalBox.classList.add('choose-deck-canonical-modal','challenger-my-decks-modal');
  document.getElementById('modal')?.classList.add('no-edge-corners-modal');
  document.getElementById('modal').classList.add('on');
}
...
function viewChallengerDeckContents(pid) {
  ...
  if(!preset) return;
  const cameFromDeckPick = !!document.querySelector('#modal .choose-deck-canonical-modal') ||
...
function renderCdbCollection() {
  ...
  if(cards.length===0){
    col.innerHTML = '<div style="color:var(--dim);font-style:italic;padding:2rem;text-align:center;width:100%;">No owned cards match this filter. Open packs to expand your collection!</div>';
    return;
  }
  cards.forEach(c=>{
  ...
  Object.entries(counts).forEach(([id,n])=>{
    ...
    row.innerHTML=`
      ...
      <span style="display:flex;align-items:center;gap:.3rem;flex-shrink:0;">
        <span style="background:rgba(18,46,24,.92);border:1px solid rgba(122,255,142,.55);color:#d9ffe0;font-family:'Cinzel',serif;font-size:.82rem;padding:.18rem .52rem;border-radius:999px;box-shadow:0 0 12px rgba(76,175,80,.24);">x${n}</span>
        <span class="rm" onclick="event.stopPropagation();cdbRemove('${id}')">Remove</span>
      </span>`;
...
function openCdbSaveAsNewDialog() {
  ...
  body.innerHTML = `
    <div class="cdb-save-top">
      <div class="cdb-save-preview">
        ...
        <div class="cdb-save-strip" id="cdb-save-strip"></div>
      </div>
  ...
  const renderPreview = () => {
    ...
    if(faceEl) faceEl.innerHTML = faceCard?.img ? `<img src="${faceCard.img}" alt="${escapeHtml(faceCard.name)}"><span>${escapeHtml(faceCard.name)}</span>` : '';
    const strip = body.querySelector('#cdb-save-strip');
    if(strip) strip.innerHTML = currentDisplay.map(id=>{
      const c = deckCards.find(card=>card.id===id);
      return c?.img ? `<div><img src="${c.img}" alt="${escapeHtml(c.name)}"></div>` : '';
    }).join('');
...
function inspectProfile(username) {
  ...
  const wr = pWins+pLosses>0 ? Math.round(pWins*100/(pWins+pLosses)) : 0;
    ...
    <div class="profile-wrap">
      ...
      <div class="profile-info">
        ...
        <div class="profile-stats">
          ...
          <div class="profile-stat"><div class="ps-label">vs Human</div><div class="ps-value" style="font-size:.9rem;">${pWins}W / ${pLosses}L</div></div>
...
function installFateCornerDock(){
  ...
  const titleActive = !!document.getElementById('s-title')?.classList.contains('active') && !document.body.classList.contains('in-game');
  if(chat) chat.style.display = titleActive ? '' : 'none';
  ...
  const chatVisible = !!(titleActive && chat && chat.style.display !== 'none');
...
function scheduleFateCornerDock(){
  ...
  if(typeof requestAnimationFrame === 'function') requestAnimationFrame(installFateCornerDock);
  setTimeout(installFateCornerDock, 80);
  setTimeout(installFateCornerDock, 260);
}
...
function sendWorldChat() {
  if(window.FATE_ONLINE_SEND_WORLD_CHAT){
    return window.FATE_ONLINE_SEND_WORLD_CHAT();
  }
  const inp = document.getElementById('wc-input');
  if(!inp) return;
  const text = inp.value.trim();
...
function initInGameChat() {
  ...
  // Hide world chat during game
  const worldChat = document.getElementById('world-chat-widget');
  if(worldChat) worldChat.style.display = 'none';
  ...
  widget.innerHTML = `
    ...
    <div class="ingame-chat-panel" id="ingame-chat-panel" style="display:none;">
      ...
      <div class="world-chat-input-row ingame-chat-input-row">
        ...
        <input type="text" class="social-chat-input" id="igc-input" placeholder="Chat..." maxlength="150" autocomplete="off" onkeydown="if(event.key==='Enter')sendInGameChat()">
...
function removeInGameChat() {
  ...
  if(el) el.remove();
  const worldChat = document.getElementById('world-chat-widget');
  if(worldChat) worldChat.style.display = '';
  if(typeof scheduleFateCornerDock === 'function') scheduleFateCornerDock();
...
function saveMatchHistory(history) {
  try { localStorage.setItem('fate_match_history', JSON.stringify(history.slice(-50))); } catch(e){}
...
function showMatchHistory(page) {
  ...
  if(history.length === 0) {
    ...
    html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:.8rem;">';
    pageItems.forEach(m => {
      ...
      html += `<div style="padding:1rem;border:1.5px solid var(--border);border-radius:12px;background:rgba(0,0,0,.35);display:flex;flex-direction:column;gap:.5rem;">
        ...
        <div style="display:flex;align-items:center;gap:.5rem;">
          ...
          <div style="font-family:Cinzel,serif;font-size:1.1rem;color:var(--gold);letter-spacing:.15em;font-weight:700;">VS</div>
          <div style="flex:1;height:1px;background:linear-gradient(to right,transparent,var(--border),transparent);"></div>
        </div>
        ${m.simulated?'<div style="text-align:center;font-size:.45rem;color:var(--dim);margin-top:-.3rem;letter-spacing:.1em;">SIMULATED</div>':''}
        <div style="display:flex;align-items:center;gap:.7rem;">
          <div style="width:56px;height:56px;border-radius:12px;overflow:hidden;background:#0a0a0f;flex-shrink:0;display:flex;align-items:center;justify-content:center;border:2px solid ${!p1Won?'#7fffa050':'#ff6b6b50'};">
...
function showDivisionPage(page, memberPage) {
  ...
  const members = LEADERBOARD.filter(e => getRank(e.elo).name === rank.name).sort((a,b) => b.elo - a.elo);
  ...
  html += `<div class="division-member-pager" style="display:flex;align-items:center;justify-content:space-between;gap:.7rem;margin:.2rem 0 .55rem;">
    ...
    <div style="font-family:Cinzel,serif;color:var(--dim);font-size:.72rem;letter-spacing:.08em;">Members ${members.length ? memberStart + 1 : 0}-${Math.min(memberStart + memberPageSize, members.length)} / ${members.length} &nbsp; Page ${_divisionMemberPageIdx + 1} / ${memberPages}</div>
  ...
  // Member list: fixed ten-slot page so sparse division pages keep the same shape.
    ...
    if(entry) {
      ...
      html += `<div class="division-pro-row is-empty" style="display:flex;align-items:center;gap:.8rem;padding:.7rem .9rem;border:1.5px solid var(--border);border-radius:10px;background:rgba(0,0,0,.3);">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div style="font-size:.72rem;color:var(--dim);margin-top:.2rem;">0W / 0L</div>
        </div>
      </div>`;
    }
  }
  ...
  daily.challenges.forEach(function(ch){
    ...
    if(curProg >= def.target){
      ...
      if(def.reward.starlight){
        ...
        if(typeof playSfx === 'function') playSfx('challengeComplete');
        if(window.toast) toast('Challenge complete: ' + def.label + ' (+ ' + def.reward.starlight + ' Starlight)');
      }
    }
  });
  if(changed){
