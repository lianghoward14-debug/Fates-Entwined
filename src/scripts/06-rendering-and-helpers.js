...
function renderHand() {
  ...
  hand.forEach((card,i)=>{
    ...
    if(canInspectHand) {
      ...
      el.onclick=()=>{
      // Own-hand clicks must select before opening detail; the modal decides
      // whether the selected card can currently be played.
      if(canActFromHand) {
...
function updatePlayerBanners() {
  ...
  if(myPicEl){
    ...
    if(typeof window.renderCanvasImage === 'function' && window.HTMLCanvasElement && _myPicSrc && !_myPicCrop) {
  ...
  if(G.aiEnabled){
    ...
    if(oppPicEl){
      ...
      if(typeof window.renderCanvasImage === 'function' && window.HTMLCanvasElement && _oppSrc && !_oppCrop) {
        let canvas = oppPicEl.querySelector(':scope > canvas.profile-pic-canvas');
        if(!canvas) {
          oppPicEl.textContent = '';
          canvas = document.createElement('canvas');
...
function getLocalActionPlayerIndex() {
  if(!G || !G._onlineRoomCode) return getPerspectivePlayerIndex();
  let idx = Number(G.localPlayerIndex);
  if(Number.isInteger(idx) && idx >= 0) return idx;
  idx = Number(G._onlinePlayerIndex);
  if(Number.isInteger(idx) && idx >= 0) return idx;
  return getPerspectivePlayerIndex();
}

function openCardDetail(card, fromHand=false, fromBoard=false) {
  if(fromBoard && typeof G !== 'undefined' && G && G._consolidating) return;
  if(!card){
    const fallbackHandPlayer = typeof getLocalActionPlayerIndex === 'function' ? getLocalActionPlayerIndex() : G.currentPlayer;
    if(G.selectedHandCard!==null) card=G.players?.[fallbackHandPlayer]?.hand?.[G.selectedHandCard];
    if(!card) return;
  }
  ...
  const handActionPlayer = getLocalActionPlayerIndex();
  const handForAction = G.players?.[handActionPlayer]?.hand || [];
  let handActionIndex = handForAction.findIndex(c=>c && card && c.iid && c.iid===card.iid);
  if(handActionIndex < 0 && Number.isInteger(G.selectedHandCard) && handForAction[G.selectedHandCard]){
    const selected = handForAction[G.selectedHandCard];
    if(!card || selected === card || (selected.iid && card.iid && selected.iid === card.iid) || (selected.id && card.id && selected.id === card.id)){
      handActionIndex = G.selectedHandCard;
      card = selected;
    }
  }
  const canUseHandCard = Number.isInteger(handActionPlayer) && handActionIndex > -1 && Number(G.currentPlayer)===Number(handActionPlayer) && String(G.phase || '').toLowerCase()==='main' && !G._isSpectator && G._onlineRole !== 'spectator';
  const isLocalHandCard = Number.isInteger(handActionPlayer) && handActionIndex > -1 && !fromBoard && !G._isSpectator && G._onlineRole !== 'spectator';
  const explainUnavailableHandAction = () => {
    if(String(G.phase || '').toLowerCase() !== 'main') return 'You can set cards during the main phase.';
    if(Number(G.currentPlayer) !== Number(handActionPlayer)) return 'It is not your turn yet.';
    return 'That card cannot be set right now.';
  };
  ...
  if((fromHand||G.selectedHandCard!==null) && isLocalHandCard){
    ...
    if(card.type==='Supporter'){
      ...
      place.dataset.unavailable = canUseHandCard ? '' : '1';
      place.title = canUseHandCard ? 'Place on Board' : explainUnavailableHandAction();
      place.onclick=()=>{
        if(!canUseHandCard){ toast(explainUnavailableHandAction()); return; }
        placeSelected();
      };
      ...
      con.dataset.unavailable = canUseHandCard ? '' : '1';
      con.title = canUseHandCard ? 'Consolidate' : explainUnavailableHandAction();
      con.onclick=()=>{
        if(!canUseHandCard){ toast(explainUnavailableHandAction()); return; }
        initiateConsolidate();
      };
  ...
  if(fromBoard && G.selectedBoardCard){
    ...
    const boardActionPlayer = getLocalActionPlayerIndex();
    const canUseBoardCard = Number.isInteger(boardActionPlayer)
      && bc.owner===boardActionPlayer
      && Number(G.currentPlayer)===Number(boardActionPlayer)
      && G.phase==='main'
      && !G._isSpectator
      && G._onlineRole !== 'spectator';
