...
function performCommand(state, ctx, command, actorIndex, options){
  ...
  if(command.type === 'ACTIVATE_LANDSCAPE'){
    ...
    if(state.landscapeId === 'igb17'){
      ...
      const token = {
        ...
        effect:String(source.card.effect || 'Copied Coordinator effect.') + ' This copied effect applies to your whole field.',
        flavor:'Tomorrow, I’ll be the same old me.',
        img:'whisper.png',
        type:'Coordinator',
  ...
  if(command.type === 'END_TURN'){
    ...
    // BH18's legacy deck-discard effect is retired; Morale resolution owns its effect.
