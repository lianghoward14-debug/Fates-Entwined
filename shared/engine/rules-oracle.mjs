...
const CARD_RULES = [
  ...
  card('bh21','Oktai','WHEN_SET','OPPONENT','OPPONENT_VIEW_OF_FIELD_FATE_AND_BOTH_MORALE_VALUES','Beginning with the opponent next turn, conceal all field Fate values and both players Morale from that opponent for exactly four opponent turns. Oktai landscape presentation remains dominant for the duration.',{duration:'FOUR_OPPONENT_TURNS_STARTING_NEXT_OPPONENT_TURN',forbidden:['CONTROLLER_VIEW_CONCEALED','ACTIVATION_TURN_COUNTS','OTHER_LANDSCAPE_REPLACES_OKTAI_PRESENTATION','STATUS_EXPIRES_EARLY']}),
