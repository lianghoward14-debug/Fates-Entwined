...
const CARD_RULES = [
  ...
  card('02','Anicka Konvicka',['WHEN_SET','PASSIVE'],'CONTROLLER','SOURCE_ZONE','Create exactly one full four-square extra safe row owned by Anicka’s controller in her zone; each friendly card placed in that generated row gains exactly +4 effective Fate, and a card consolidated into Anicka’s generated row costs exactly 2 less Reinforcement.',{stateEvidence:['SAFE_ROW_ADDED.playerIndex_EQUALS_SOURCE_CONTROLLER','FOUR_PLAYABLE_EXTRA_SQUARES_HAVE_SOURCE_CONTROLLER','ROW_OWNER_IDENTICAL_IN_BOTH_SEAT_PROJECTIONS','GENERATED_ROW_PLACEMENT_BONUS_IS_PLUS_4','ANICKA_ROW_REINFORCEMENT_COST_MINUS_2'],forbidden:['SAFE_ROW_ASSIGNED_TO_OPPONENT','VIEWER_INDEX_CHANGES_ROW_OWNER','FATE_BONUS_OUTSIDE_ANICKA_ROW','COST_REDUCTION_OUTSIDE_ANICKA_ROW','COST_REDUCTION_EXCEEDS_2','ANICKA_QUALIFIES_AS_A_NEW_PLACEMENT_MORE_THAN_ONCE']}),
  ...
  card('04','Zoe','WHEN_SET','CONTROLLER','ONE_SQUARE_IN_SOURCE_ZONE','Opponent cards occupying the chosen square cannot leave the field unless immune to Zoe’s effect.',{cardinality:'EXACTLY_ONE_OPEN_OR_OCCUPIED_SQUARE',duration:'REST_OF_GAME',forbidden:['BLOCKS_CONTROLLER_CARD','BLOCKS_PLACEMENT','IMMUNE_CARD_LOCKED','TARGET_OUTSIDE_SOURCE_ZONE']}),
  ...
  card('100','Felicyta and Květka (Youth)','PASSIVE','CONTROLLER','SOURCE_CARD','Gain +2 permanent Fate once for each controller turn during which Snow on the Carpathians is active; independently gain +5 effective Fate if controller controls another Felicyta or Květka card.',{prerequisites:['SNOW_COUNTER_REQUIRES_IGB15_ON_CONTROLLER_TURN','KINSHIP_REQUIRES_OTHER_QUALIFYING_CARD'],forbidden:['SOURCE_QUALIFIES_ITSELF_FOR_PLUS_5','OPPONENT_QUALIFYING_CARD_COUNTS','PLUS_2_TICKS_ON_OPPONENT_TURN','PLUS_2_TICKS_WITHOUT_SNOW']}),
  ...
  card('18','1st US Marines','WHEN_SET','CONTROLLER','OPPONENT_NEXT_TURN_SUPPORTER_EFFECTS','Suppress opponent Supporter set effects during exactly the opponent’s next turn.',{duration:'OPPONENT_NEXT_TURN_ONLY',forbidden:['CONTROLLERS_EFFECTS_SUPPRESSED','SUPPRESSION_PERSISTS_EXTRA_TURN','USE_COUNTER_CREATED','BLOCKED_SUPPORTER_LEAVES_PENDING_EFFECT']}),
  ...
  card('20','South Wind Spearman','ACTIVATE','CONTROLLER','OPPONENT_NEXT_TURN_MORALE_DAMAGE','At any time, activate Shield Wall so that during exactly the opponent’s next turn, that opponent inflicts 0 Morale Damage.',{useLimit:'TWO_USES_PER_CARD',duration:'OPPONENT_NEXT_TURN_ONLY',forbidden:['AUTOMATIC_WHEN_SET_ACTIVATION','CONTROLLERS_DAMAGE_BLOCKED','STATUS_PERSISTS_EXTRA_TURN','ONLY_NEXT_CALCULATION_BLOCKED','THIRD_USE']}),
  ...
  card('31','Oathbound Noble Fighter','WHEN_SET','SOURCE_EFFECT','ONE_EFFECT_MUTABLE_CARD_IN_SOURCE_ZONE','Selected card loses exactly 4 permanent Fate, clamped at zero.',{cardinality:'EXACTLY_ONE_IF_AVAILABLE',forbidden:['TARGET_OUTSIDE_ZONE','CANCEL_OR_CLOSE_APPLIES_TO_SOURCE_AUTOMATICALLY','MULTIPLE_TARGETS','FATE_BELOW_ZERO']}),
  ...
  card('40','Christopher Erbs','ACTIVATE','CONTROLLER','NEXT_CARD_CONTROLLER_DRAWS','Arm the next draw; that card gains exactly +7 permanent Fate, then consume the arm.',{useLimit:'TWO_USES_PER_CARD',duration:'UNTIL_NEXT_CONTROLLER_DRAW',forbidden:['ARMED_TWICE_SIMULTANEOUSLY','OPPONENT_DRAW_CONSUMES','MORE_THAN_PLUS_7','STATUS_NOT_CONSUMED']}),
  ...
  card('42','West German Soldier','WHEN_SET','CONTROLLER','CONTROLLER_DECK_THEN_CONTROLLER_HAND','Draw exactly three available cards, then require exactly three legal hand discards; resolution remains pending until selection completes.',{cardinality:'DRAW_THREE_THEN_DISCARD_THREE',forbidden:['DISCARD_BEFORE_DRAW_PRESENTATION','OPPONENT_HAND_USED','CLOSING_PICKER_AUTO_SELECTS','TURN_ENDS_WITH_PENDING_SELECTION','LEGACY_DRAW_TWO_DISCARD_TWO_ALSO_APPLIED']}),
  ...
  card('58','Crossroads Worker','WHEN_SET','CONTROLLER','ONE_SUPPORTER_IN_CONTROLLER_DISCARD','If an eligible Supporter exists, pay exactly 15 Morale and move the selected Supporter to controller hand; with no eligible Supporter, skip without paying, activation cinematic, or modal.',{cardinality:'EXACTLY_ONE_IF_AVAILABLE',forbidden:['EMPTY_DISCARD_PAYS_COST','EMPTY_DISCARD_OPENS_EFFECT_CINEMATIC','NON_SUPPORTER_SELECTED','OPPONENT_DISCARD_SEARCHED']}),
  ...
  card('61','Maria Song','WHEN_SET','CONTROLLER','ONE_REVEALED_CHARACTER_NAME_FROM_OPPONENT_HAND','Reveal opponent Characters; chosen card identity causes every copy in opponent hand, deck, and field to lose exactly 9 permanent Fate, clamped at zero.',{forbidden:['NON_CHARACTER_SELECTED','CONTROLLERS_COPIES_LOSE','DISCARD_COPIES_LOSE_IF_TEXT_EXCLUDES_DISCARD','SAME_COPY_MUTATED_TWICE']}),
  ...
  card('64','Cook Islands Duelist','WHEN_SET','CONTROLLER','CONTROLLERS_NEXT_MORALE_DAMAGE_CALCULATION','Arm Blade Dance when set. At the next Morale Damage Calculation, double the complete Morale Damage inflicted by the controller, then consume the effect.',{cardinality:'NEXT_CALCULATION_ONCE',forbidden:['FATE_MODIFIED','OPPONENT_DAMAGE_DOUBLED','MORE_THAN_ONE_CALCULATION_PER_INSTANCE']}),
  ...
  card('69','Breakfast Republic Busser','WHEN_SET','CONTROLLER','UP_TO_ONE_INITIATOR_IN_CONTROLLER_DISCARD','Pay 25 Morale, then add the selected Initiator from controller discard to controller hand.',{forbidden:['MOVEMENT_GRANTED','STATUS_CREATED','INITIATOR_MORALE_HEALED','NON_INITIATOR_SELECTED','OPPONENT_DISCARD_SEARCHED']}),
  ...
  card('75','The Ledger-keepers','WHEN_SET','CONTROLLER','TOP_FIVE_CARDS_OF_CONTROLLER_DECK','Reveal up to five available top cards and let the controller rearrange those exact instances. Future draws consume this order; searches remove their selected instance without disturbing the remaining cards.',{forbidden:['OLD_SUPPORTER_COPY_EFFECT_EXECUTED','DECK_REMAINDER_REORDERED','CARD_DUPLICATED_OR_LOST','DRAWS_RANDOMIZE_REARRANGED_ORDER']}),
  card('76','ALPINE Infantry',['WHEN_SET','PASSIVE'],'CONTROLLER','SOURCE_CARD','On set gain exactly +5 permanent Fate; source cannot be discarded, cannot count for bonuses, is effect-immutable, and cannot be used for consolidation.',{forbidden:['OTHER_EFFECT_MUTATES_SOURCE','SOURCE_DISCARDED','SOURCE_COUNTS_FOR_ANY_CONDITION_OR_AURA','SOURCE_USED_AS_REINFORCEMENT','OWN_INTRINSIC_PLUS_5_BLOCKED']}),
  ...
  card('89','Zsofia Szocs (Youth)','PASSIVE','CONTROLLER','SOURCE_CARD','Gain exactly +8 effective Fate while controller has activated fewer than 10 Supporter effects this game; lose bonus at count 10.',{forbidden:['OPPONENT_EFFECT_COUNT_USED','TEN_STILL_QUALIFIES','PLACEMENT_WITH_SKIPPED_EFFECT_COUNTS_AS_ACTIVATION']}),
  ...
  card('91','Wodny Potok Villager','HAND_ARRIVAL','CONTROLLER','LANDSCAPE_EFFECT_CARD_IN_DECK','Whenever added to hand, search a card mentioning landscape, add it to hand and reduce its cost by 2, minimum zero.',{forbidden:['CREATES_LANDSCAPE_LOCK','WRONG_SEARCH_TARGET']}),
  ...
  card('95','Carpathian Specter','PASSIVE','CONTROLLER','SOURCE_CARD','From turn 14, each completed controller Draw phase grants +1 permanent Fate. No lifetime cap. Engineer can force one +1 proc.',{forbidden:['BEFORE_TURN_14','OPPONENT_DRAW_PHASE','BONUS_FOR_EACH_CARD_DRAWN']}),
  ...
  card('bh04','Anicka Konvicka (Selva Island)','WHEN_SET','CONTROLLER','ALL_EFFECT_MUTABLE_OPPONENT_CARDS_OF_DECLARED_TYPE_IN_SOURCE_ZONE','Split exactly 24 permanent Fate loss evenly using established integer distribution among eligible opponent cards of declared type.',{forbidden:['CONTROLLERS_CARDS_LOSE','WRONG_TYPE_OR_ZONE_LOSES','TOTAL_LOSS_EXCEEDS_AVAILABLE_FATE_OR_24','DISTRIBUTION_REROLLS_ON_RENDER']}),
  ...
  card('bh08','Maja Kaminska (University)','PASSIVE','CONTROLLER','ALL_CARDS_CONTROLLER_CONTROLS_IN_SOURCE_ZONE','Each time controller actually negates or suppresses an opponent-controlled effect, every controlled card in source zone gains exactly +2 permanent Fate once.',{forbidden:['CONTROLLERS_OWN_EFFECT_TRIGGERS','DECLINED_REACTION_TRIGGERS','OPPONENT_REACTION_TRIGGERS','CARD_OUTSIDE_ZONE_GAINS','SAME_REACTION_TRIGGERS_TWICE']}),
  ...
  card('bh10','Francisek','WHEN_SET','CONTROLLER','SUPPORTER_CATALOG','Create one selected Supporter from the full catalog in the controller hand. Its next set bypasses the ordinary set limit, consumes that exemption, and still counts toward the global Supporter cap. This is neither a search nor a draw.',{forbidden:['DECK_SEARCHED','DRAW_EFFECT_ACTIVATED','HAND_DISCARDED','GLOBAL_SUPPORTER_CAP_BYPASSED']}),
  card('bh11','Felicyta Janowicz (University)','PASSIVE','CONTROLLER','ALL_CONTROLLED_ADJACENCY_BONUSES_IN_SOURCE_ZONE','Each active Superior Marks source adds one original copy of every positive numeric ADJACENCY_BONUS controlled in its zone; multiple sources stack additively, while penalties and non-bonus adjacency effects remain unchanged.',{forbidden:['OPPONENT_ADJACENCY_BONUS_DOUBLED','ADJACENCY_PENALTY_DOUBLED','NON_BONUS_ADJACENCY_EFFECT_DOUBLED','BONUS_OUTSIDE_SOURCE_ZONE_DOUBLED']}),
  ...
  card('bh13','Hugh Roberts','WHEN_SET','CONTROLLER','UP_TO_THREE_EFFECT_MUTABLE_CARDS_IN_CONTROLLER_HAND','Choose zero to three eligible cards in controller hand; each selected card gains exactly +7 permanent Fate, then returns from controller hand to controller deck.',{cardinality:'ZERO_TO_THREE',forbidden:['OPPONENT_HAND_CARD_SELECTED','MORE_THAN_THREE_CARDS_SELECTED','IMMUNE_CARD_MUTATED_OR_MOVED','FATE_GAIN_APPLIED_AFTER_CARD_LEAVES_HAND','SELECTED_CARD_GAINS_OTHER_THAN_7','SELECTED_CARD_SENT_TO_DISCARD']}),
  ...
  card('bh17','Jakob Eltzholtz','PASSIVE','CONTROLLER','NEWLY_CONSOLIDATED_CARD','If controller has more total Fate than opponent when consolidation completes, the consolidated card gains exactly 3 permanent Fate per active source.',{forbidden:['SET_CARD_TRIGGERS','TIED_OR_LOSING_PLAYER_TRIGGERS','SOURCE_CARD_RECEIVES_BONUS_INSTEAD']}),
  card('bh18','Jimmy (Post-Cynthia Hug)','PASSIVE','OPPONENT','OPPONENT_ZONE_FATE_IN_SOURCE_ZONE','Whenever controller would take Morale Damage from a Morale Calculation, reduce the opponent\'s Zone Fate in this card\'s zone by exactly 3 per active source.',{forbidden:['NON_CALCULATION_MORALE_DAMAGE_TRIGGERS','CONTROLLER_ZONE_FATE_REDUCED','WRONG_ZONE_REDUCED','SOURCE_OVERLAY_OUTSIDE_MORALE_CALCULATION']}),
  card('bh19','Abed','WHEN_SET','CONTROLLER','ALL_POSITIVE_PERMANENT_FATE_GAIN_EFFECTS_THIS_TURN','For the rest of this owner turn, High-T adds one extra copy of the original permanent Fate gain; only one High-T effect can be active per player and additional copies do not stack.',{forbidden:['EXPONENTIAL_DOUBLING','FATE_LOSS_DOUBLED','STATUS_SURVIVES_OWNER_TURN_END']}),
  card('bh20','Makenna (Bird Cult)','WHEN_SET','CONTROLLER','MATCH_TURN_LIMIT','When set, increase the match turn limit by exactly two turns. Each separately set copy adds another two turns.',{forbidden:['TURN_LIMIT_REDUCED','OTHER_THAN_TWO_TURNS_ADDED','COPY_DOES_NOT_STACK']}),
  card('bh21','Oktai','WHEN_SET','OPPONENT','OPPONENT_VIEW_OF_MATCH_INFORMATION','Beginning with the opponent next turn, conceal field and zone Fate, both players Morale and Morale changes, both hand counts, the timer, turn count, and all status banners except The Vast Taklamakan from that opponent for exactly four opponent turns. Oktai landscape presentation remains dominant for the duration.',{duration:'FOUR_OPPONENT_TURNS_STARTING_NEXT_OPPONENT_TURN',forbidden:['CONTROLLER_VIEW_CONCEALED','ACTIVATION_TURN_COUNTS','OTHER_LANDSCAPE_REPLACES_OKTAI_PRESENTATION','OTHER_STATUS_BANNER_VISIBLE','STATUS_EXPIRES_EARLY']}),
  card('bh22','Jaime','WHEN_SET_AND_EVERY_MORALE_CALCULATION','CONTROLLER','ONE_SQUARE_IN_CONTROLLERS_SAFE_ROW','Choose exactly one square in controller safe row. At every Morale calculation, recover Morale equal to the Fate of the card on that square and track total Morale recovered.',{forbidden:['SQUARE_OUTSIDE_CONTROLLER_SAFE_ROW','OPPONENT_RECEIVES_RECOVERY','EMPTY_SQUARE_RECOVERS_MORALE','TRACKER_COUNTS_OTHER_RECOVERY']}),
  card('bh23','Panacea Militia','WHEN_SET','CONTROLLER','ONE_TRIGGERED_FATE_COORDINATOR_CONTROLLER_CONTROLS_IN_SOURCE_ZONE','Choose Zsofia Szocs, Joie, or Maja Kaminska (University) in source zone; source permanently gains exactly that Coordinator\'s accumulated per-target triggered Fate history from earlier triggers this game.',{cardinality:'EXACTLY_ONE_IF_AVAILABLE',forbidden:['COORDINATOR_OUTSIDE_SOURCE_ZONE_SELECTED','OTHER_COORDINATOR_SELECTED','FUTURE_TRIGGERS_COUNTED_RETROACTIVELY','TOTAL_ACROSS_ALL_TARGETS_USED','CURRENT_POTENCY_MULTIPLIED_BY_TRIGGER_COUNT']}),
  card('bh24','Leningrad 4th Rifles','WHEN_SET','CONTROLLER','CONTROLLERS_NEXT_SUPPORTER_SET','Pay exactly 15 Morale, then the next Supporter controller sets does not contribute to the ordinary Supporter set allowance and gains exactly +4 permanent Fate; the universal five-Supporter hard cap still applies and the grant is consumed once.',{duration:'UNTIL_NEXT_CONTROLLER_SUPPORTER_SET',forbidden:['OPPONENT_SUPPORTER_CONSUMES_STATUS','NON_SUPPORTER_CONSUMES_STATUS','MORE_THAN_PLUS_4','STATUS_APPLIES_MORE_THAN_ONCE','MORALE_COST_NOT_PAID','GLOBAL_SUPPORTER_HARD_CAP_BYPASSED']}),
  card('bh25','ALPINE Engineer','WHEN_SET','CONTROLLER','VALID_CONDITIONAL_TRIGGERED_FATE_SOURCES_ON_YOUR_FIELD','At turn 18 or later, proc each valid conditional triggered Fate-gain effect on cards you control across all zones once. Each triggered effect retains its own targeting zone. Present the Engineer overlay on every triggered source card. Fate-gain modifiers do not count as source triggers, and effects requiring a presently nonexistent target do not proc.',{forbidden:['OPPONENT_CONTROLLED_SOURCE_PROCS','ACTIVATES_BEFORE_TURN_18','HSEI_LING_MODIFIER_PROCS','JAKOB_WITHOUT_CONSOLIDATED_TARGET_PROCS','OVERLAY_ON_ENGINEER_INSTEAD_OF_TRIGGERED_SOURCE']})
];
...
const LANDSCAPE_RULES = [
  ...
  landscape('igb9','Californian Sunsets: West Coast Dreaming','OUTSIDE_DRAW_PHASE_CARD_DRAWN','DRAWING_PLAYER','Once per qualifying draw effect, drawing player may select any mutable field card to gain exactly +3 permanent Fate, even if that effect draws multiple cards.', ['DRAW_PHASE_TRIGGERS','PER_CARD_MULTI_DRAW_TRIGGERS','OPPONENT_CHOSES','EMPTY_TARGET_LEAVES_PENDING_PROMPT']),
  ...
  landscape('igb21','Moffitt Library: Whiteboard Drawings','ACTIVE_PLAYER_ACTION','ACTING_PLAYER','Once per game during own turn, choose one to four non-Star cards from the complete catalog and add fresh instances to hand.', ['STAR_CARD_ADDED','MORE_THAN_FOUR_ADDED','SECOND_USE','CARD_ADDED_TO_OPPONENT_HAND']),
  landscape('igb22','The Mexican Civil War, 2029: The Chihuahuan Desert','SETUP_AND_CONSOLIDATION','BOTH_PLAYERS','Select exactly two distinct deterministic-random zones at setup; consolidating any card into either selected zone costs exactly +1 Reinforcement.', ['SAME_ZONE_SELECTED_TWICE','UNSELECTED_ZONE_TAXED','MORE_THAN_PLUS_1','SET_CARD_TAXED']),
  landscape('igb23','Pacifica Nights: The Shores of La Helena',['CONTINUOUS','MORALE_CALCULATION'],'BOTH_PLAYERS','Card-effect Morale payments cost zero. At each Morale Calculation, each player recovers Morale equal to exactly twice their current hand size, capped at maximum Morale.', ['MORALE_DAMAGE_PREVENTED','NON_COST_LOSS_WAIVED','WRONG_HAND_COUNT','HEAL_EXCEEDS_MAXIMUM']),
  landscape('igb24','The Moscow Protest, 2025: Dawn of a Revolution','START_OF_TURN_20','TENURE_QUALIFIED_SUPPORTERS','When Turn 20 begins, each Supporter that has been on the board for at least 10 turns gains exactly +6 permanent Fate once and becomes immune to opponent effects. Other Supporters gain neither benefit.', ['RESOLVES_BEFORE_TURN_20','YOUNG_SUPPORTER_GAINS','YOUNG_SUPPORTER_IMMUNE','NON_SUPPORTER_GAINS','NON_SUPPORTER_IMMUNE','SAME_CARD_GAINS_TWICE'])
];
...
export function expectedEffectiveFateFromOracle(state, cardIid){
  ...
  function calculate(target){
    ...
    if(oracleSourceActive(state, target) && selfId === '89' && Number(state?.supporterEffectsActivated?.[controller] || 0) < 10) modifier += 8;
