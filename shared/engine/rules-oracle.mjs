...
const CARD_RULES = [
  ...
  card('20','South Wind Spearman','WHEN_SET','CONTROLLER','OPPONENT_NEXT_TURN_MORALE_DAMAGE','During exactly the opponent’s next turn, that opponent inflicts 0 Morale Damage.',{duration:'OPPONENT_NEXT_TURN_ONLY',forbidden:['CONTROLLERS_DAMAGE_BLOCKED','STATUS_PERSISTS_EXTRA_TURN','ONLY_NEXT_CALCULATION_BLOCKED']}),
  ...
  card('bh18','Jimmy (Post-Cynthia Hug)','PASSIVE','OPPONENT','ONE_RANDOM_CARD_FROM_ENDING_PLAYERS_DECK','When opponent ends their turn, discard one deterministic-random card from that opponent deck per active source and reveal its identity to both players.',{forbidden:['CONTROLLERS_DECK_DISCARDED','HAND_CARD_DISCARDED','SOURCE_OVERLAY_ON_DISCARDED_CARD']}),
