(function(){
  ...
  function drawTributeCue(ctx, r, state){
    ...
    const color = ready ? 'rgba(126,232,255,1)' : selected ? 'rgba(255,246,111,1)' : placement ? 'rgba(255,232,86,1)' : 'rgba(255,228,65,1)';
    const fill = ready ? 'rgba(70,202,255,.33)' : selected ? 'rgba(255,226,62,.34)' : placement ? 'rgba(255,215,0,.30)' : 'rgba(255,215,0,.23)';
    const radius = Math.max(5, Math.min(10, r.w * .06));
    const outerPad = (selected || ready || placement) ? 6.4 : 5.2;
    ctx.save();
    ctx.shadowColor = ready ? 'rgba(104,214,255,.92)' : 'rgba(255,220,70,.88)';
    ctx.shadowBlur = ready ? 30 : 26;
    roundedPath(ctx, r.x - outerPad, r.y - outerPad, r.w + outerPad * 2, r.h + outerPad * 2, radius + 2);
    ctx.fillStyle = fill;
    ctx.fill();
    ctx.lineWidth = ready ? 4.6 : (selected ? 4.3 : (placement ? 4 : 3.7));
    ctx.strokeStyle = color;
    ctx.stroke();
    ctx.shadowBlur = 0;
    ctx.globalAlpha = .78;
    ctx.strokeStyle = ready ? 'rgba(222,248,255,.95)' : 'rgba(255,251,199,.92)';
    ctx.lineWidth = 1.2;
    roundedPath(ctx, r.x - outerPad + 4, r.y - outerPad + 4, r.w + outerPad * 2 - 8, r.h + outerPad * 2 - 8, radius + 1);
    ctx.stroke();
    ctx.globalAlpha = 1;
  ...
  function drawTopCornerRails(ctx, r, color, alpha){
    const len = Math.max(26, Math.min(58, r.w * .10));
    const inset = 12;
    const y = r.y + inset;
    ctx.save();
    ctx.globalAlpha = alpha == null ? 1 : alpha;
  ...
  function drawZonePanel(ctx, zone, zoneRect, headerRect, snapshot){
    const ownerTint = zone.z === 0 ? 'rgba(12,38,62,.28)' : zone.z === 1 ? 'rgba(60,45,22,.24)' : 'rgba(18,50,42,.27)';
    const panelGrad = ctx.createLinearGradient(zoneRect.x, zoneRect.y, zoneRect.x, zoneRect.y + zoneRect.h);
    panelGrad.addColorStop(0, 'rgba(3,5,9,.82)');
    panelGrad.addColorStop(.38, ownerTint);
    panelGrad.addColorStop(1, 'rgba(1,3,7,.84)');
    ...
    innerGlow.addColorStop(0, 'rgba(255,226,126,.04)');
    ...
    ctx.fill();
    ctx.lineWidth = 1.2;
    ctx.strokeStyle = 'rgba(255,220,82,.76)';
    ctx.stroke();

    ctx.save();
    ctx.globalAlpha = .86;
    ctx.strokeStyle = 'rgba(255,222,96,.46)';
    ctx.lineWidth = .85;
    roundedPath(ctx, zoneRect.x + 7, zoneRect.y + 7, zoneRect.w - 14, zoneRect.h - 14, 6);
    ctx.stroke();
    ctx.restore();
    drawTopCornerRails(ctx, zoneRect, 'rgba(255,219,77,.90)', .95);
  ...
  function drawZoneCellPerimeter(ctx, zone, originX, originY, scrollY, clip){
    ...
    const pad = 5;
    const r = {x:minX - pad, y:minY - pad, w:maxX - minX + pad * 2, h:maxY - minY + pad * 2};
    ctx.save();
    roundedPath(ctx, r.x, r.y, r.w, r.h, 7);
    ctx.strokeStyle = 'rgba(255,222,96,.34)';
    ctx.lineWidth = 1.3;
    ctx.stroke();
    roundedPath(ctx, r.x + 3, r.y + 3, r.w - 6, r.h - 6, 5);
    ctx.strokeStyle = 'rgba(255,246,184,.13)';
    ctx.lineWidth = 1;
    ctx.stroke();
    ctx.restore();
  }
  ...
  function drawHoverCue(ctx, hit){
    ...
    const r = cardLikeCell ? hit.cardRect : hit.rect;
    ...
    if(hit.kind === 'card' || cardLikeCell){
      ...
      roundedPath(ctx, r.x + 6, r.y + 6, r.w - 12, r.h - 12, 5);
      ctx.lineWidth = good ? 1.25 : 2;
      ctx.strokeStyle = good ? 'rgba(255,236,142,.52)' : 'rgba(255,101,115,.78)';
      ctx.stroke();
      ctx.fillStyle = good ? 'rgba(236,203,101,.035)' : 'rgba(255,80,92,.08)';
      ctx.fill();
    }
    ctx.restore();
  }
  ...
  function drawHandPanel(ctx, handRect, snapshot, count){
    ...
    bg.addColorStop(0, 'rgba(22,18,12,.88)');
    bg.addColorStop(.42, 'rgba(7,8,13,.94)');
    bg.addColorStop(1, 'rgba(2,3,7,.96)');
    ctx.fillStyle = bg;
    ctx.fill();
    ctx.lineWidth = 1.45;
    ctx.strokeStyle = 'rgba(255,220,92,.74)';
    ctx.stroke();
    roundedPath(ctx, r.x + 10, r.y + 10, r.w - 20, r.h - 20, 6);
    ctx.strokeStyle = 'rgba(255,237,157,.30)';
    ctx.lineWidth = 1;
    ctx.stroke();
    drawCornerRails(ctx, {x:r.x + 8, y:r.y + 8, w:r.w - 16, h:r.h - 16}, 'rgba(255,226,105,.78)', .95);
    ...
    tabGrad.addColorStop(0, 'rgba(20,18,12,.98)');
    tabGrad.addColorStop(1, 'rgba(3,4,8,.96)');
  ...
  function drawCommandButton(ctx, hitMap, command, label, rect, opts){
    ...
    if(primary){
      grad.addColorStop(0, disabled ? 'rgba(54,54,58,.92)' : 'rgba(186,139,45,.96)');
      grad.addColorStop(.48, disabled ? 'rgba(36,38,42,.94)' : 'rgba(90,61,23,.97)');
      grad.addColorStop(1, disabled ? 'rgba(20,22,26,.98)' : 'rgba(16,15,15,.98)');
    } else if(danger) {
      grad.addColorStop(0, disabled ? 'rgba(31,34,39,.88)' : 'rgba(82,30,29,.94)');
      grad.addColorStop(1, disabled ? 'rgba(14,16,20,.94)' : 'rgba(16,8,10,.96)');
    } else {
    ...
    ctx.stroke();
    ctx.save();
    ctx.globalAlpha = disabled ? .16 : .34;
    ctx.strokeStyle = primary ? 'rgba(255,246,184,.42)' : 'rgba(255,236,159,.22)';
    ctx.beginPath();
    ctx.moveTo(rect.x + 11, rect.y + 7);
    ctx.lineTo(rect.x + rect.w - 11, rect.y + 7);
    ctx.moveTo(rect.x + 11, rect.y + rect.h - 7);
    ctx.lineTo(rect.x + rect.w - 11, rect.y + rect.h - 7);
    ctx.stroke();
    ctx.restore();
    ctx.fillStyle = disabled ? 'rgba(200,200,192,.48)' : (danger ? '#ffc4b8' : '#f2d778');
    ctx.font = '800 ' + Math.max(9, Math.round(rect.h * .29)) + 'px Cinzel, serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
  ...
  function drawCommandDock(ctx, hitMap, snapshot, cssW, cssH){
    const interaction = snapshot && snapshot.interaction ? snapshot.interaction : {};
    const canConsolidate = interaction.consolidating || (interaction.currentPlayer === snapshot.viewer && (interaction.phase || '') === 'main');
    const buttons = [
      {command:'end-turn', label:'END TURN', primary:true, disabled:interaction.currentPlayer !== snapshot.viewer, h:38}
    ];
    if(canConsolidate) buttons.push({command:'consolidate', label:interaction.consolidating ? 'STOP' : 'CONSOLIDATE', active:interaction.consolidating, disabled:false, h:34});
    buttons.push({command:'end-game', label:'END GAME', danger:true, disabled:false, h:34});
    buttons.push({command:'audio', label:'AUDIO', disabled:false, h:28});
    const w = 142;
    const pad = 13;
    const gap = 10;
    const headerH = 34;
    const h = headerH + pad * 2 + buttons.reduce(function(total, btn){ return total + btn.h; }, 0) + gap * Math.max(0, buttons.length - 1);
    const chatReserveW = 170;
    const x = Math.max(12, cssW - chatReserveW - w - 18);
    const y = Math.max(92, cssH - h - 26);
    const r = {x, y, w, h};
    ctx.save();
    roundedPath(ctx, x, y, w, h, 9);
    const bg = ctx.createLinearGradient(x, y, x, y + h);
    bg.addColorStop(0, 'rgba(19,16,10,.93)');
    bg.addColorStop(.52, 'rgba(6,8,13,.96)');
    bg.addColorStop(1, 'rgba(1,2,6,.98)');
    ctx.fillStyle = bg;
    ctx.fill();
    ctx.strokeStyle = 'rgba(236,203,101,.70)';
    ctx.lineWidth = 1.35;
    ctx.stroke();
    ctx.strokeStyle = 'rgba(255,247,194,.18)';
    ctx.lineWidth = 1;
    roundedPath(ctx, x + 6, y + 6, w - 12, h - 12, 8);
    ctx.stroke();
    drawTopCornerRails(ctx, {x:x + 4, y:y + 4, w:w - 8, h:h - 8}, 'rgba(255,223,91,.70)', .9);
    ctx.fillStyle = '#f2d778';
    ctx.font = '900 9px Cinzel, serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('COMMAND', x + w / 2, y + 22);

    const btnX = x + pad;
    const btnW = w - pad * 2;
    let btnY = y + headerH + pad;
    hitMap.uiCommands.push({kind:'ui-command', command:'dock', disabled:true, rect:r});
    buttons.forEach(function(btn){
      drawCommandButton(ctx, hitMap, btn.command, btn.label, {x:btnX, y:btnY, w:btnW, h:btn.h}, btn);
      btnY += btn.h + gap;
    });
    ctx.restore();
  }
