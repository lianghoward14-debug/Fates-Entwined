(function(){
  ...
  function buildProductionBoardMetrics(vp, opts){
    ...
    return {
      ...
      rowLabelGap,
      boardPadX,
      boardPadTop:Number.isFinite(opts.boardPadTop) ? opts.boardPadTop : rem * 0.22,
      boardPadBottom:Number.isFinite(opts.boardPadBottom) ? opts.boardPadBottom : rem * 0.18,
      boardGap,
      zonePadX,
      zonePadTop:Number.isFinite(opts.zonePadTop) ? opts.zonePadTop : rem * 1.58,
      zonePadBottom:Number.isFinite(opts.zonePadBottom) ? opts.zonePadBottom : rem * 0.92,
      cardOffsetY:Number.isFinite(opts.cardOffsetY) ? opts.cardOffsetY : 0,
      headerTrackH:Number.isFinite(opts.headerTrackH) ? opts.headerTrackH : 42,
  ...
  function buildPeripheralLayout(snap, vp, metrics){
    ...
    const cardW = clamp(winW * 0.056, 88, 116);
    const cardH = Math.round(cardW * 1.4);
    const handCount = handCards.length;
    const handFallbackW = Math.min(1240, Math.max(760, winW - 600));
    const desiredHandRect = rect((winW - handFallbackW) / 2, winH - cardH - 42, handFallbackW, cardH + 30);
    const domHandRect = elementViewportRect('#hand-cards', desiredHandRect);
    const handRect = rect(desiredHandRect.x, domHandRect.y || desiredHandRect.y, desiredHandRect.w, Math.max(domHandRect.h || 0, desiredHandRect.h));
    const maxHandW = Math.max(1, Math.min(handRect.w - 36, winW - 560));
    ...
    const baseOppCardW = hasRevealedOppCards
      ? clamp(winW * 0.041, 52, 70)
      : clamp(winW * 0.034, 34, 54);
